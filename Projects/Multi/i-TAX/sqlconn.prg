...
*!*					do conn with mvu_backend,mvu_server,mvu_user,mvu_pass,_sqldbname
				Rval = this.sqlconnects(mvu_backend,mvu_server,mvu_user,mvu_pass,_sqldbname,_sqlconhandle)
				do case
				case type('rVal') = "C"
					if rval = "Y"
						mexit = .t.
...
*--------------------*
	...
	case _mvu_backend = "1" && SQL server
		...
		if &_sqlconhandle < 0
			ans=messagebox("Cannot Establish connection to SQL Server "+alltrim(upper(_mvu_server))+chr(13)+chr(13)+"Please check the Server Name/User Id/Password",5+32+256,vumess)
			if ans = 4
				return "Y"
			else
				return "N"
