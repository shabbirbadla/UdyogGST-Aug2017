...
/* 
Modified Date: 17 Mar, 2010
Modified By :  Shrikant S. 
Reason		:  For removing  'IR' Records against 'AR'
Modified Date: 12 May, 2010
Modified By :  Shrikant S. 
Reason		:  For consider of non-excise bill in part-1
Modified Date: 19 May, 2010
Modified By :  Shrikant S. 
Reason		:  For consider of shortage stock in part-1

Modified Date: 07 Mar, 2010
Modified By :  Shrikant S. 
Reason		:  For Bug-2042 for current month purchase

Modified Date: 27 Aug, 2010
Modified By :  Vasant
Reason		:  To show Original Purchase Qty. instead of Warehouse Transfer Qty.

--Modified Date: 31/12/2011
--Modified By :  Vasant
--Reason		:  Bug 1348 - RG Page No. should generate from start (i.e.) for each financial year
--Comment where Changes done : Bug1348

--Modified Date: 18/01/2012
--Modified By :  Vasant
--Reason		:  Bug 879 - Warehouse Transfer
--Comment where Changes done : Bug879

--Modified Date: 07/04/2012
--Modified By :  Vasant
--Reason		:  Issue in RG 23 D and FORM 2 after Updating BUG-1348
--Comment where Changes done : Bug3421

--Modified Date: 11/04/2012
--Modified By :  Vasant
--Reason		:  In FORM 2 efiling for Importer instead of ECC No the Application should check for IEC No.
--Comment where Changes done : Bug3499
...
Declare @FCON as NVARCHAR(4000),@SQLCOMMAND as NVARCHAR(4000),@ColOrder as Nvarchar(4000),@InsFlds as Nvarchar(4000) --vasant 13/02/10
	...
	Declare @Rg23cpur as bit,@Rg23flag as bit,@CommBill Varchar(200),@ShortageStk Varchar(500),@RgPg_ReSet as bit	----Bug1348
	Declare @Sta_Dt SMALLDATETIME,@End_Dt SMALLDATETIME	--Bug3421
	Select Top 1 @Sta_Dt = Sta_dt,@End_Dt = End_dt From Vudyog..Co_mast Where Dbname = Db_name() And Sta_Dt <= @SDATE Order by Sta_Dt Desc	--Bug3421
...
Declare @dateFilter Varchar(200)				--Added by Shrikant S. on 07/03/2012 for Bug-2042
set @dateFilter=' AND a.date <='''+CONVERT(VARCHAR(50),@EDATE)+''' '	--Added by Shrikant S. on 07/03/2012 for Bug-2042

	Set @InsFlds = ''		--vasant 13/02/10

	Set @Rg23cpur = 0
	SET @Rg23flag = 0
	Set @RgPg_ReSet =  0		----Bug1348
...
-- 11/02/10
SET @FCON=CASE WHEN LEN(RTRIM(LTRIM(@FCON)))=0 THEN ' WHERE ' ELSE @FCON+' AND ' END ---added BY SHRIKANT s. ON 130310
...
--**-- @VSDATE=null	@VDTFLD = null
	Select Top 1 @ShortageStk=case when isnull(form2ss,0)=0 then 'and (g.entry_ty in (''DC'',''IR'') Or g.bcode_nm in (''DC'',''IR''))' else 'and (g.entry_ty in (''DC'',''IR'',''SS'') Or g.bcode_nm in (''DC'',''IR'',''SS''))' end from manufact --Added by Shrikant S. on 19/05/2010 
	Select top 1 @CommBill=case when isnull(commbill,0)=0 then '''EXCISE''' else '''EXCISE'',''NON-EXCISE''' end from manufact --Added by Shrikant S. on 12/05/2010 
	select top 1 @Rg23cpur = rg23cpur from manufact
	select top 1 @Rg23flag = rg23flag from manufact		--vasant 140210
	select top 1 @RgPg_ReSet = RgPg_ReSet from manufact		----Bug1348
...
--**-- ,c.u_choice as csuppinv
	...
	SET @SQLCOMMAND = 'select b.tran_cd as ctran_cd,b.entry_ty as centry_ty,b.itserial as citserial,
		...
		left join it_mast
		on a.it_code = it_mast.it_code '+RTRIM(@FCON)+' c.[rule] in ('+@CommBill+')'+RTRIM(@ShortageStk)+@dateFilter		--Added by Shrikant S. on 07/03/2012 for Bug-2042
		--on a.it_code = it_mast.it_code '+RTRIM(@FCON)+' c.[rule] in ('+@CommBill+')'+RTRIM(@ShortageStk) --Changed by Shrikant S. on 19/05/2010 --Commented By Shrikant S. on 07/03/2012 for Bug-2042
		--and	(g.entry_ty in (''DC'',''IR'',''SS'') Or g.bcode_nm in (''DC'',''IR'',''SS''))'  --Changed by Shrikant S. on 19/05/2010 
		--on a.it_code = it_mast.it_code '+RTRIM(@FCON)+' c.[rule] in (''EXCISE'',''NON-EXCISE'') --Changed by Shrikant S. on 12/05/2010 
...
/*
	...
	SET @SQLCOMMAND = 'select printit=cast(0 as bit),a.*,
		...
		Case when isnull(c.ettype,'''')='''' then h.vend_type else c.ettype end as ettype,(case when isnull(c.u_pinvno,'' '') = '' '' then c.inv_no else c.u_pinvno end) as u_pinvno,(case when isnull(c.u_pinvdt,'' '') = '' '' then c.date else (case when year(c.u_pinvdt) <= 1900 then c.date else c.u_pinvdt end) end) as u_pinvdt,
		...
		on b.entry_Ty = a.pentry_ty and b.tran_Cd = a.ptran_Cd and b.itserial = a.pitserial '+RTRIM(@FCON)+' c.[rule] in ('+@CommBill+')
		and (g.entry_ty in (''AR'',''IR'') or g.bcode_nm in (''AR'',''IR'') or (g.entry_ty in (''GT'') And c.u_sinfo=0) or (g.bcode_nm in (''GT'') And c.u_sinfo=0))'		
		--on b.entry_Ty = a.pentry_ty and b.tran_Cd = a.ptran_Cd and b.itserial = a.pitserial '+RTRIM(@FCON)+' c.[rule] in (''EXCISE'',''NON1-EXCISE'') --Changed by Shrikant S. on 12/05/2010
*/
	SET @SQLCOMMAND = ''
	SET @SQLCOMMAND = 'select printit=cast(0 as bit),a.*,
		b.tran_cd,b.entry_ty,b.date,b.itserial,
		...
		c.cons_id as scons_id,c.scons_id as sscons_id,
		h.mailname as sname,e.add1 as sadd1,e.add2 as sadd2,e.add3 as sadd3,e.city as scity,e.zip as szip,
		e.range as srange,e.division as sdivision,e.coll as scoll,e.eccno as seccno,e.iec_no as siec_no,space(100) as sexregno,
		b.qty as balqty,b.examt as balexamt,b.u_cessamt as balcesamt,b.u_cvdamt as balcvdamt,b.u_hcesamt as balhcsamt,
		b.qty as srqty,b.examt as srexamt,b.u_cessamt as srcesamt,b.u_cvdamt as srcvdamt,b.u_hcesamt as srhcsamt,
		b.qty as popqty,b.examt as popexamt,b.u_cessamt as popcesamt,b.u_cvdamt as popcvdamt,b.u_hcesamt as pophcsamt
		INTO '+@TBLNAME2+' FROM tradeitem b 
...
--**--
   ...
   ----Bug1348
	...
	begin
		SET @SQLCOMMAND = ''
		SET @SQLCOMMAND = 'Update  '+@TBLNAME2+' Set Rgpage = b.NPgNo From Gen_SrNo b,  '+@TBLNAME2+' a '
		SET @SQLCOMMAND = @SQLCOMMAND +' where a.entry_ty=b.entry_ty and a.tran_cd = b.tran_cd and a.itserial = b.itserial '
		SET @SQLCOMMAND = @SQLCOMMAND +' and a.cdate Between b.sta_dt and b.end_dt'
		EXECUTE SP_EXECUTESQL @SQLCOMMAND
	end	
----Bug1348
...
--**--
	...
	SET @SQLCOMMAND = 'update '+@TBLNAME2+' set sadd1 = e.add1,sadd2 = e.add2,sadd3 = e.add3,scity = e.city,szip = e.zip,
		srange = e.range,sdivision = e.division,scoll = e.coll,seccno = e.eccno,siec_no = e.iec_no,sexregno = e.exregno
		from '+@TBLNAME2+' a,ac_mast e 
		where a.scons_id = e.ac_id and isnull(a.sscons_id,0) = 0'		--Bug3499
	EXECUTE SP_EXECUTESQL @SQLCOMMAND
...
*/
		...
		SET @SQLCOMMAND = 'select a.entry_ty,a.tran_cd,a.itserial,
			...
			isnull(a.popcvdamt,0) - sum(isnull(a.ccvdamt,0)) + sum(isnull(a.srcvdamt,0)) as balcvdamt,
			isnull(a.pophcsamt,0) - sum(isnull(a.chcesamt,0)) + sum(isnull(a.srhcsamt,0)) as balhcsamt
			INTO '+@TBLNAME3+' from '+@TBLNAME2+' a where a.cdate < '''+CONVERT(VARCHAR(50),@STA_DT)+''' 
			group by a.entry_ty,a.tran_cd,a.itserial,a.popqty,a.popexamt,a.popcesamt,a.popcvdamt,a.pophcsamt '
		EXECUTE SP_EXECUTESQL @SQLCOMMAND
		...
		SET @SQLCOMMAND = ''
		SET @SQLCOMMAND = 'delete from '+@TBLNAME2+' where cqty > 0 and Cdate not between '''+CONVERT(VARCHAR(50),@STA_DT)+''' and '''+CONVERT(VARCHAR(50),@EDATE)+''' '
		EXECUTE SP_EXECUTESQL @SQLCOMMAND
--Bug1348
	end
...
*/
	Set @InsFlds = 'printit,tran_cd,entry_ty,date,itserial,
		...
		scons_id,sscons_id,
		sname,sadd1,sadd2,sadd3,scity,szip,
		srange,sdivision,scoll,seccno,siec_no,sexregno,
		balqty,balexamt,balcesamt,balcvdamt,balhcsamt,
		srqty,srexamt,srcesamt,srcvdamt,srhcsamt,
		popqty,popexamt,popcesamt,popcvdamt,pophcsamt,
		rgpage1,rgpage2,ppageno'
...
*/
	...
	SET @SQLCOMMAND = ''
	SET @SQLCOMMAND = 'select * into '+@TBLNAME1+' from '+@TBLNAME3+' where entry_ty = ''IR'''
	EXECUTE SP_EXECUTESQL @SQLCOMMAND

--Bug879 (Start)
/*
	SET @SQLCOMMAND = ''
	SET @SQLCOMMAND = 'DECLARE @COLNO AS INT,@ENTRY_TY AS VARCHAR(2),@TRAN_CD AS INT,@ITSERIAL AS VARCHAR(10)
	DECLARE @MANUAC_ID AS INT,@MANUSAC_ID AS INT,@MANUBILL AS VARCHAR(20),@MANUDATE AS SMALLDATETIME
	DECLARE WARE_NM CURSOR FOR SELECT COLNO,ENTRY_TY,TRAN_CD,ITSERIAL,MCONS_ID,MSCONS_ID,MANUBILL,MANUDATE FROM '+@TBLNAME1+' ORDER BY ENTRY_TY,TRAN_CD,ITSERIAL
	OPEN WARE_NM
	FETCH NEXT FROM WARE_NM INTO @COLNO,@ENTRY_TY,@TRAN_CD,@ITSERIAL,@MANUAC_ID,@MANUSAC_ID,@MANUBILL,@MANUDATE
	WHILE @@FETCH_STATUS=0
	BEGIN

		DROP TABLE '+@TBLNAME2+'

		select a.u_choice as ssuppinv,a.u_pinvno,a.u_pinvdt,
			qty = (case when b.billqty != 0 then b.billqty else b.qty end),
			examt = (case when b.billqty != 0 then b.billexamt else b.examt end),
			u_cessamt = (case when b.billqty != 0 then b.billcesamt else b.u_cessamt end),
			u_cvdamt = (case when b.billqty != 0 then b.billcvdamt else b.u_cvdamt end),
			u_hcesamt = (case when b.billqty != 0 then b.billhcsamt else b.u_hcesamt end)
			into '+@TBLNAME2+'
			from trademain a,tradeitem b,litemall c,manu_det d
			where a.entry_ty = b.entry_ty and a.tran_cd = b.tran_cd
			and b.entry_ty = c.pentry_ty and b.tran_cd = c.ptran_cd and b.itserial = c.pitserial
			and b.entry_ty = d.entry_ty and b.tran_cd = d.tran_cd and b.itserial = d.itserial
			and c.entry_ty = @entry_ty and c.tran_cd = @tran_cd and c.itserial = @itserial 
			and d.manuac_id = @manuac_id and d.manusac_id = @manusac_id and d.manubill = @manubill and d.manudate = @manudate
			
		UPDATE '+@TBLNAME3+' Set 
			qty = b.qty, examt = b.examt, u_cessamt = b.u_cessamt,u_cvdamt = b.u_cvdamt,u_hcesamt = b.u_hcesamt,
			ssuppinv = b.ssuppinv,u_pinvno = b.u_pinvno,u_pinvdt = b.u_pinvdt from '+@TBLNAME3+' a,'+@TBLNAME2+' b where Colno = @COLNO
...
*/
	SET @SQLCOMMAND = ''
	SET @SQLCOMMAND = 'DECLARE @COLNO AS INT,@ENTRY_TY AS VARCHAR(2),@TRAN_CD AS INT,@ITSERIAL AS VARCHAR(10)
	DECLARE WARE_NM CURSOR FOR SELECT COLNO,ENTRY_TY,TRAN_CD,ITSERIAL FROM '+@TBLNAME1+' ORDER BY ENTRY_TY,TRAN_CD,ITSERIAL
	OPEN WARE_NM
	FETCH NEXT FROM WARE_NM INTO @COLNO,@ENTRY_TY,@TRAN_CD,@ITSERIAL
	WHILE @@FETCH_STATUS=0
	BEGIN
		...
		select a.u_choice as ssuppinv,a.u_pinvno,a.u_pinvdt,
			...
			u_hcesamt = (case when b.billqty != 0 then b.billhcsamt else b.u_hcesamt end)
			into '+@TBLNAME2+'
			from trademain a,tradeitem b,litemall c
			where a.entry_ty = b.entry_ty and a.tran_cd = b.tran_cd
			and b.entry_ty = c.pentry_ty and b.tran_cd = c.ptran_cd and b.itserial = c.pitserial
			and c.entry_ty = @entry_ty and c.tran_cd = @tran_cd and c.itserial = @itserial 
...
/*
		select top 1 a.u_choice as ssuppinv,a.u_pinvno,a.u_pinvdt,
			qty = (case when b.billqty != 0 then b.billqty else b.qty end),
			examt = (case when b.billqty != 0 then b.billexamt else b.examt end) as examt,
			u_cessamt = (case when b.billqty != 0 then b.billcesamt else b.u_cessamt end) as u_cessamt,
			u_cvdamt = (case when b.billqty != 0 then b.billcvdamt else b.u_cvdamt end) as u_cvdamt,
			u_hcesamt = (case when b.billqty != 0 then b.billhcsamt else b.u_hcesamt end) as u_hcesamt
			into '+@TBLNAME2+'
			from trademain a,tradeitem b
			where a.entry_ty = b.entry_ty and a.tran_cd = b.tran_cd
...
/*
	...
	*/
	...
	Set @ColOrder = 'ware_nm,cdate,cinv_no,
		case when rtrim(rgpage1) != '' '' then CONVERT(numeric(10),rgpage1) else 0 end,
		case when rtrim(rgpage2) != '' '' then CONVERT(numeric(10),rgpage2) else 0 end,
		cspageno,csentno,u_pinvdt,u_pinvno'				--vasant 14/02/10 --vasant 27/08/10
...
--Added by Shrikant S. On 17 Mar, 2010---------------Start
	SET	@SQLCOMMAND = 'Delete from '+@TBLNAME3+' Where Centry_ty=''IR''' 	
	EXECUTE SP_EXECUTESQL @SQLCOMMAND
--Added by Shrikant S. On 17 Mar, 2010---------------Start
	...
	SET @SQLCOMMAND = ''
	SET @SQLCOMMAND = 'alter table '+@TBLNAME3+' drop column Colno'
	EXECUTE SP_EXECUTESQL @SQLCOMMAND


	SET @SQLCOMMAND = ''
	--SET @SQLCOMMAND = 'select * from '+@TBLNAME3+@EXTPAR+' order by '+@ColOrder	--vasant 13/02/10
	SET @SQLCOMMAND = 'select Colno=IDENTITY (int, 1, 1),* into '+@TBLNAME4+' from '+@TBLNAME3+@EXTPAR+' order by '+@ColOrder	--vasant 13/02/10
	EXECUTE SP_EXECUTESQL @SQLCOMMAND

	SET @SQLCOMMAND = ''
	SET @SQLCOMMAND = 'select min(a.colno) as colno,a.centry_ty,a.ctran_cd,a.citserial,a.entry_ty,a.tran_cd,a.itserial into '+@TBLNAME1+' from '+@TBLNAME4+' a
		group by a.centry_ty,a.ctran_cd,a.citserial,a.entry_ty,a.tran_cd,a.itserial'
	EXECUTE SP_EXECUTESQL @SQLCOMMAND

	SET @SQLCOMMAND = ''
	SET @SQLCOMMAND = 'update '+@TBLNAME4+' set printit = 1 
		from '+@TBLNAME4+' a,'+@TBLNAME1+' b 
		where a.colno = b.colno and a.centry_ty = b.centry_ty and a.ctran_cd = b.ctran_cd and a.citserial = b.citserial 
			and a.entry_ty = b.entry_ty and a.tran_cd = b.tran_cd and a.itserial = b.itserial'
    EXECUTE SP_EXECUTESQL @SQLCOMMAND

	SET @SQLCOMMAND = ''
	SET @SQLCOMMAND = 'select * from '+@TBLNAME4
