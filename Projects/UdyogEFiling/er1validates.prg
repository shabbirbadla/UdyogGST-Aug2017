...
IF _RepoSDate >= CTOD('01/10/2011')
	IF TYPE('lst_exc.exmclearty') != 'C'
		_errmessage = 'Please update New ER1 Format'
	ELSE
...
IF _RepoSDate >= CTOD('01/10/2012') AND ALLTRIM(_RepList.rep_nm) != 'ER1OCT12'
	_errmessage = 'Please update New ER1 Format'
Endif
...
Case mProduct = 'VU9'
	*select * from lst_exc group by entry_ty,date,doc_no,itserial where qm = '3' and e_name = _me_name into cursor _lst_exc1	&&vasant191010
	...
	IF USED('_lst_exc1')
		...
		SELECT _lst_exc1
		SCAN
			UPDATE lst_exc SET u_arrears = ALLTRIM(u_arrears)+Padl(ALLTRIM(STR(_msr)),3,'0') WHERE ChapNo = _lst_exc1.ChapNo AND RateUnit = _lst_exc1.RateUnit AND qm = '3' AND srno = 'A'
			_msr = _msr + 1
			SELECT _lst_exc1
		Endscan	
		USE IN _lst_exc1
	...
	IF USED('_lst_exc1')
		...
		SELECT _lst_exc1
		SCAN
			UPDATE lst_exc SET u_arrears = ALLTRIM(u_arrears)+Padl(ALLTRIM(STR(_msr)),2,'0') WHERE Eit_Name = _lst_exc1.Eit_Name AND qm = '3' AND srno = 'A'
			_msr = _msr + 1
			SELECT _lst_exc1
		Endscan	
		USE IN _lst_exc1
	...
	IF USED('_lst_exc1')
		...
		SELECT _lst_exc1
		SCAN
			UPDATE lst_exc SET u_arrears = ALLTRIM(u_arrears)+Padl(ALLTRIM(STR(_msr)),2,'0') WHERE ExmClearTy = _lst_exc1.ExmClearTy AND qm = '3' AND srno = 'A'
			_msr = _msr + 1
			SELECT _lst_exc1
		Endscan	
		USE IN _lst_exc1
	...
	IF USED('_lst_exc1')
		...
		SELECT _lst_exc1
		SCAN
			UPDATE lst_exc SET u_arrears = ALLTRIM(u_arrears)+Padl(ALLTRIM(STR(_msr)),2,'0') WHERE E_Name = _lst_exc1.E_Name AND Rule  = _lst_exc1.Rule AND qm = '3' AND srno = 'A'
			_msr = _msr + 1
			SELECT _lst_exc1
		Endscan	
		USE IN _lst_exc1
...
&& Commented by Shrikant S. on 20/11/2015 for Bug-27246		&& Start
...
*!*					SELECT _lst_exc2
*!*					SCAN
*!*						UPDATE lst_exc SET u_arrears = ALLTRIM(u_arrears)+Padl(ALLTRIM(STR(_msr)),2,'0') WHERE qm = '3' AND srno = 'A' AND pmkey = '-' AND ;
...
*!*			USE IN _lst_exc1
*!*		ENDIF
&& Commented by Shrikant S. on 20/11/2015 for Bug-27246		&& End
...
*!*		ENDIF
	...
	SELECT MAX(U_Arrears) As U_Arrears,QM,ChapNo,RateUnit,Eit_Name,ExmClearTy,E_Name,Rule,E_Per FROM lst_exc ;
		INTO CURSOR lst_exc1 WHERE pmkey = '-' AND SrNo = 'A' GROUP BY QM,ChapNo,RateUnit,Eit_Name,ExmClearTy,E_Name,Rule,E_Per
	IF USED('lst_exc1')
		UPDATE lst_exc SET u_arrears = lst_exc1.U_Arrears from lst_exc,lst_exc1 WHERE ;
			lst_exc.ChapNo = lst_exc1.ChapNo And lst_exc.RateUnit = lst_exc1.RateUnit And lst_exc.Eit_Name = lst_exc1.Eit_Name ;
			And lst_exc.ExmClearTy = lst_exc1.ExmClearTy And lst_exc.E_Name = lst_exc1.E_Name And lst_exc.Rule = lst_exc1.Rule and lst_exc.pmkey <> '-'
	ENDIF
...
*!*		SELECT chapno,eit_name,rateunit,rule,e_name,e_per,u_arrears FROM lst_exc WHERE qm = '3' AND srno != 'A' GROUP BY chapno,eit_name,rateunit,rule,e_name,e_per INTO CURSOR _lst_exc1
*!*		IF USED('_lst_exc1')
...
If mProduct = 'VU9'
	...
	GO Top
	LOCATE FOR ALLTRIM(QM) == '4'
	IF !FOUND()
		APPEND BLANK IN lst_exc
		REPLACE QM WITH '4' IN lst_exc
	Endif	
	LOCATE FOR ALLTRIM(QM) == '5'
	IF !FOUND()
		APPEND BLANK IN lst_exc
		REPLACE QM WITH '5' IN lst_exc
	Endif	
	...
	Endif	
	LOCATE FOR ALLTRIM(QM) == '9C'
	IF !FOUND()
		APPEND BLANK IN lst_exc
		REPLACE QM WITH '9C',SRNo WITH 'A',INV_SR WITH 'ABCDE' IN lst_exc
	Endif	
	LOCATE FOR ALLTRIM(QM) == '9D'
	IF !FOUND()
		APPEND BLANK IN lst_exc
		REPLACE QM WITH '9D',SRNo WITH 'A',INV_SR WITH 'ABCDE' IN lst_exc
	Endif	
	SELECT lst_exc
	GO Top
ENDIF 
...
UPDATE xmldata_Vw SET BlankTag = .t. WHERE ALLTRIM(taggroup) == '4'
UPDATE xmldata_Vw SET BlankTag = .t. WHERE ALLTRIM(taggroup) == '5'
...
UPDATE xmldata_Vw SET BlankTag = .t. WHERE ALLTRIM(taggroup) == '9C' AND Empty(ALLTRIM(MLINE(FInvoice9C,1)))
UPDATE xmldata_Vw SET BlankTag = .t. WHERE ALLTRIM(taggroup) == '9D' AND Empty(ALLTRIM(MLINE(FInvoice9D,1)))
