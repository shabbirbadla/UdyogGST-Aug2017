...
/****** Object:  StoredProcedure [dbo].[USP_REP_INPUTTOPRODUCTION]    Script Date: 02/05/2010 10:05:48 ******/
SET ANSI_NULLS ON
GO
...
CREATE  PROCEDURE [dbo].[USP_REP_INPUTTOPRODUCTION] 
...
SET @SQLCOMMAND='select IPITEM.ENTRY_TY,IPITEM.DOC_NO,IPITEM.item_no,IPITEM.inv_no,IPITEM.date,IT_MAST.it_NAME,IPITEM.inv_sr,IPITEM.qty,IT_MAST.p_unit,IPITEM.dept,PTMAIN.[RULE],IRMAIN.[RULE] from IPITEM'
SET @SQLCOMMAND=@SQLCOMMAND+'JOIN IPMAIN ON (IPITEM.ENTRY_TY=IPMAIN.ENTRY_TY AND IPITEM.TRAN_CD=IPMAIN.TRAN_CD)'
SET @SQLCOMMAND=@SQLCOMMAND+'JOIN IT_MAST ON (IPITEM.ITEM=IT_MAST.IT_NAME)'
SET @SQLCOMMAND=@SQLCOMMAND+'LEFT JOIN IPITREF ON (IPITEM.ENTRY_TY=IPITREF.ENTRY_TY  AND IPITEM.DATE=IPITREF.DATE AND IPITEM.DOC_NO=IPITREF.DOC_NO AND IPITEM.ITSERIAL=IPITREF.ITEM_NO) '
