           ...
           string outextension = string.Empty;
            ...
            try
            {
                ...
                if (Isdigsign == true)
                {
                    ...
                    Sqlstring = "select FileData,Passw from DigitalSign where dept='" + Dept + "' and cate='" + Cate + "' and invsr='" + Inv_sr + "' and validity like '%" + Entry_ty + "%' and ('" + Inv_dt.ToString("yyyy/MM/dd") + "'>=convert(smalldatetime,sdate) and '" + Inv_dt.ToString("yyyy-MM-dd") + "'<=convert(varchar(10),edate,103))";

                    m_obj_dr = cls_Sqlhelper.ExecuteReader(sqlconn, CommandType.Text, Sqlstring);

                    if (m_obj_dr.HasRows == true && m_obj_dr != null)
                    {
                        ...
                        while (m_obj_dr.Read())
                        {
                            Path = new MemoryStream((byte[])m_obj_dr["FileData"]);
                            Pass = CryptorEngine.Decrypt(m_obj_dr["Passw"].ToString(), true);
