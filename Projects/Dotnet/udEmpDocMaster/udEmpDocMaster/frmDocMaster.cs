...
{
    public partial class frmDocMaster : uBaseForm.FrmBaseForm
    {

        DataAccess_Net.clsDataAccess oDataAccess;
        DataSet dsMain;
        string SqlStr = string.Empty;
        string vMainField = "DocNm", vMainTblNm = "Emp_Doc_Master", vMainFldVal = "";
        ...
        public frmDocMaster(string[] args)
        {
            this.pDisableCloseBtn = true;  /* close disable  */  /*Ramya Bug-8354*/
            InitializeComponent();
            this.pPApplPID = 0;
            this.pPara = args;
            this.pFrmCaption = "Document Master";
        ...
        private void frmDocMaster_Load(object sender, EventArgs e)
        {
            ...
            {
                this.btnDocName.Image = Image.FromFile(fName);
        ...
        {
            this.txtDocName.DataBindings.Clear();
        ...
        private void mthBindData()
        {
            ...
            this.txtDocName.DataBindings.Add("Text", dsMain.Tables[0], "DocNm");
        ...
        private void mthEnableDisableFormControls()
        {
            ...
            {
                vEnabled = true;
                this.btnDocName.Enabled = false;
                
            }
            else
            {
                this.btnDocName.Enabled = true;
               
            }

            if (this.pAddMode)
            {
                this.txtDocName.Enabled = true;
            }
            else
            {
        ...
        private void mthChkSaveValidation(ref Boolean vValid)
        {
            ...
            {
                MessageBox.Show("Document Name Could not Blank", this.pPApplText, MessageBoxButtons.OK, MessageBoxIcon.Stop);
                vValid = false;
            }
            this.txtDocName.Focus();
        }
        ...
        private void btnDocName_Click(object sender, EventArgs e)
        {
            mcheckCallingApplication();   
            string VForText = string.Empty, vSearchCol = string.Empty, strSQL = string.Empty, Vstr = string.Empty, vColExclude = string.Empty, vDisplayColumnList = string.Empty, vReturnCol = string.Empty;
            DataSet tDs = new DataSet();
            SqlStr = "Select DocNm,SortOrd from "+vMainTblNm+" order by SortOrd";
            tDs = oDataAccess.GetDataSet(SqlStr, null, vTimeOut);

            DataView dvw = tDs.Tables[0].DefaultView;

            VForText = "Select Document Code";
            vSearchCol = "DocNm";
            vDisplayColumnList = "DocNm:Document Name,SortOrd:Sort Order";
            vReturnCol = "DocNm";
            ...
            vMainFldVal = dsMain.Tables[0].Rows[0]["DocNm"].ToString().Trim();
            if (oSelectPop.pReturnArray != null)
            {
                this.txtDocName.Text = oSelectPop.pReturnArray[0];
                vMainFldVal = this.txtDocName.Text.Trim();
        ...
        private void txtDocName_Validating(object sender, CancelEventArgs e)
        {
            
                DataSet dsData = new DataSet();
                string sqlstr;
                sqlstr = " select " + vMainField + " from " + vMainTblNm + " where " + vMainField + "='" + txtDocName.Text.Trim() + "'";
                if (this.pEditMode)
                {
                ...
                if (dsData.Tables[0].Rows.Count > 0)
                {
                    ...
                    string ErrMsg = t.GetToolTip(txtDocName).ToString();
                    errorProvider.SetError(txtDocName, "Duplicate " + lblDoc.Text + " value");
                    txtDocName.Focus();
                    cValid= false; 
                    e.Cancel = true;
                    return;
            ...
            errorProvider.SetError(txtDocName, "");
        }

        private void frmDocMaster_FormClosed(object sender, FormClosedEventArgs e) 
        {
