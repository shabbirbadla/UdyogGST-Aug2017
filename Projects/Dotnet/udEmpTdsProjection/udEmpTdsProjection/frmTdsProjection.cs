...
namespace udEmpTdsProjection
{
    public partial class frmTdsProjection : uBaseForm.FrmBaseForm
    {
        ...
        DataSet vDsCommon;
        short vTimeOut = 60; /*Changed 20 to 60 Ramya*/
        DataTable dtMain;
        //int vId;
        ...
        private void mInsertProcessIdRecord()/*Added Rup 07/03/2011*/
        {
            ...
            cAppName = "udEmpTdsProjection.exe";
        ...
        {
            mcheckCallingApplication(); /*Ramya 26/02/2013*/
            this.pAddMode = false;
            this.pEditMode = false;
            this.mthEnableDisableFormControls();
            this.mthChkNavigationButton();
        ...
        {
            mcheckCallingApplication();  /*Ramya 26/02/13*/
            Boolean IsValid=true;
            DateTime vStaDt;
            string ymdDate1, ymdDate2;
            ...
            //string ymdDate;
            //vStaDt = ((DateTime)this.vDsCommon.Tables["company"].Rows[0]["Sta_Dt"]);
            //ymdDate = vStaDt.Year.ToString().Trim() + "/" + ((DateTime)this.vDsCommon.Tables["company"].Rows[0]["Sta_Dt"]).Month.ToString().Trim() + "/" + "01";
            //vPrintPara = vPrintPara + ",'" + ymdDate + "'";
            //vStaDt = ((DateTime)this.vDsCommon.Tables["company"].Rows[0]["End_Dt"]);
            //ymdDate = vStaDt.Year.ToString().Trim() + "/" + ((DateTime)this.vDsCommon.Tables["company"].Rows[0]["end_Dt"]).Month.ToString().Trim() + "/" + "28";
            //vPrintPara = vPrintPara + ",'" + ymdDate + "'";
            
            this.StbLbl.Text = "Running TDS Projection....";
            this.stbMain.Refresh();
            this.StbLbl.Visible = true;
            this.stbMain.Refresh();
           
            DataTable tDt = new DataTable();
            int count = 0;  /*Ramya 01/03/13*/
            ...
            if (IsValid)
            {
                ...
                foreach (DataRow drt in dtMain.Rows)
                {
                    if ((Boolean)drt["ColProcess"])
                    {
                        ...
                        vStaDt = ((DateTime)this.vDsCommon.Tables["company"].Rows[0]["Sta_Dt"]);
                        ymdDate1 = vStaDt.Year.ToString().Trim() + "/" + ((DateTime)this.vDsCommon.Tables["company"].Rows[0]["Sta_Dt"]).Month.ToString().Trim() + "/" + "01";
                        vStaDt = ((DateTime)this.vDsCommon.Tables["company"].Rows[0]["End_Dt"]);
                        ymdDate2 = vStaDt.Year.ToString().Trim() + "/" + ((DateTime)this.vDsCommon.Tables["company"].Rows[0]["end_Dt"]).Month.ToString().Trim() + "/" + "28";
                        SqlStr = "set dateformat ymd Execute Usp_Ent_Emp_TDS_Projection '" + this.txtYear.Text.Trim() + "','em.EmployeeCode in (''" + drt["EmployeeCode"].ToString().Trim() + "'')'," + (checkBox1.Checked == false ? "1" : "0");
                        SqlStr = SqlStr + ",'" + ymdDate1 + "','" + ymdDate2 + "'";
                        //MessageBox.Show(oDataAccess.serverName);
                        //MessageBox.Show(SqlStr);
                        tDt = oDataAccess.GetDataTable(SqlStr, null, vTimeOut);
                        count++;/*Ramya 01/03/13*/
                    }
                ...
                if (count == 0)
                {
                    MessageBox.Show("Please select at least one employee for Process", this.pPApplText, MessageBoxButtons.OK, MessageBoxIcon.Information);   /*Ramya 01/03/13*/
                    this.StbLbl.Text = "";
                    return;
                }
            ...
            this.StbLbl.Text = "TDS Projection Process Completed";
            this.stbMain.Refresh();
            this.StbLbl.Text = "";
            this.stbMain.Text = "";
            this.stbMain.Refresh();
        ...
        private void mthPrint(Int16 vPrintOption)
            {
            this.btnTDSProjection.Focus();
            this.dgrMain.Refresh();
            string vPrintPara = "";
            this.mthPrintPara(ref vPrintPara);
            //MessageBox.Show(vPrintPara);
            this.StbLbl.Text = "Generating Report....";
            this.stbMain.Refresh();
            this.stbMain.Refresh();

            if (vPrintPara != "")
            {
                string vRepGroup = "EMPLOYEETDSPROJECTION";
                ...
                oPrint.pSpPara = vPrintPara + ",'" + this.pAppUerName.Trim() + "'"; /*Ramya 26/02/13*/
        ...
        private void mthPrintPara(ref string vPrintPara)
        {
            ...
            if (vPrintPara.Length > 1)
            {
                ...
                vPrintPara = " 'em.EmployeeCode in (" + vPrintPara +")'";
