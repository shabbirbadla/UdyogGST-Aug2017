...
namespace QueryCommander.Database.Microsoft.Sql2000
{
	...
	public class DataManager : IDatabaseManager
	{
		...
		public DataTable ExecuteCommand (string command, IDbConnection  dataConnection)
		{
			...
			try 
			{
				dataCommand = new SqlCommand(command, (SqlConnection)dataConnection);	
		...
		public DataSet ExecuteCommand_DataSet (string command, IDbConnection dataConnection, string databaseName)
		{
			...
			try 
			{	
				_currentCommand = new SqlCommand(command, (SqlConnection)dataConnection);
		...
		public ArrayList GetDatabasesObjects(string ServerName, IDbConnection dataConnection)
		{
			...
			try 
			{
				dataCommand = new SqlCommand(QueryString, (SqlConnection)dataConnection);	
				dataAdapter = new SqlDataAdapter(dataCommand);
...
//				// Collect parameters
//				if(row.ItemArray[1].ToString().ToUpper().Trim()=="P" ||
//					row.ItemArray[1].ToString().ToUpper().Trim()=="FN" ||
//					row.ItemArray[1].ToString().ToUpper().Trim()=="TF")
//				{
//					DataTable dtColumns = ExecuteCommand("select name + '=?' from syscolumns where id = "+row.ItemArray[2].ToString() ,dataConnection);
//					string var="";
//					foreach(DataRow rowVar in dtColumns.Rows)
//						var+=rowVar.ItemArray[0].ToString() + ", ";
//
//					if(var.Length>0)
//					{
//						var=var.Substring(0,var.Length-2);
//						_parameterToolTip = "Parameters: " + var;
//					}
//				}
//				else
