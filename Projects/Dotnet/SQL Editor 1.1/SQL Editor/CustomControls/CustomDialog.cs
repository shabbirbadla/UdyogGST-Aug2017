...
{
	/// <summary>
	/// Summary description for CustomSaveDialog.
	/// Code from CodeProject.Com Article:
	...
	public class CustomSaveDialog
	{
		...
		private XmlWriteMode m_WriteMode;
		...
		private int HookProc(int hdlg, int msg, int wParam, int lParam)
		{
			switch (msg)
			{
				case WM_INITDIALOG:
					...
					// create the label
					int labelHandle = CreateWindowEx(0, "STATIC", "mylabel", WS_VISIBLE | WS_CHILD | WS_TABSTOP,
						point.X, point.Y + 16, 200, 100, parent, 0, 0, 0);
					SetWindowText(labelHandle, "&XML Export Type:");
					...
					int comboHandle = CreateWindowEx(0, "ComboBox", "mycombobox", WS_VISIBLE | WS_CHILD | CBS_HASSTRINGS | CBS_DROPDOWNLIST | WS_TABSTOP,
						point.X, point.Y + 8, rightPoint.X - point.X, 100, parent, 0, 0, 0);
					SendMessage(comboHandle, WM_SETFONT, fontHandle, 0);

					// and add the XML output mode we want to offer
					// order of next three lines is important
					SendMessage(comboHandle, CB_ADDSTRING, 0, XmlWriteMode.WriteSchema.ToString());
					SendMessage(comboHandle, CB_ADDSTRING, 0, XmlWriteMode.IgnoreSchema.ToString());
					SendMessage(comboHandle, CB_ADDSTRING, 0, XmlWriteMode.DiffGram.ToString());
				
					SendMessage(comboHandle, CB_SETCURSEL, (int)m_WriteMode, 0);
				...
				case WM_NOTIFY:
					...
					if (nmhdr.Code == CDN_FILEOK)
					{
						...
						m_WriteMode = (XmlWriteMode)SendMessage(m_ComboHandle, CB_GETCURSEL, 0, 0);
					}
					break;
			}
			return 0;
		...
		public XmlWriteMode xmlWriteMode
		{
			get
			{
				return m_WriteMode;
		...
		public DialogResult ShowDialog()
		{
			...
			ofn.hwndOwner = _handle; //Form.ActiveForm.Handle;
		
			// we need to find out the active screen so the dialog box is
			// centred on the correct display
			m_ActiveScreen = Screen.FromHandle(ofn.hwndOwner);
