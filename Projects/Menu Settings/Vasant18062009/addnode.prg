If INLIST(dec(GlobalObj.getpropertyval("udProdCode")),'VudyogMFG','VudyogTRD')
...
********************************************************************
					...
					If Used('deltemp')
						Do While .T.
							Select * From deltemp Where deltemp.rng = rng Into Cursor _cdelrng
							Select _cdelrng
							If Reccount() > 0
								_uj = _uj + 1
							Else
...
*	inps=INPUTBOX("Type Node Caption:","Name","Text")
				If !Empty(inps)
					...
					If Used('deltemp')
						Do While .T.
							Select * From deltemp Where rng = _uj Into Cursor _cdelrng
							Select _cdelrng
							If Reccount() > 0
								_uj = _uj + 1
							Else
