    ...
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
           ...
           // MessageBox.Show("Loaded...");
            ...
            ////Added by Priyanka B on 18/05/2017 Start
            //if (dsGloble.Tables["QC_Process_Master"].Rows.Count != 0)
            //{
            //    NavButtonState(true, true, false, false);
            //    //Databind(0);
            //}
            ////Added by Priyanka B on 18/05/2017 End

            //NavButtonState(true, true, false, false);//<--- Added By Pankaj On 28/04/2014 for Bug-22827  //Commented by Priyanka B on 18/05/2017
        ...
        private void ControlState(Mode md)
        {
            if (md == Mode.View)
            {
                ...
                else if (dsGloble.Tables["QC_Process_Master"].Rows.Count == 0 || dsGloble.Tables["QC_Process_Master"].Rows.Count == 1)
                //else if (dgvParam.Rows.Count == 0 || dgvParam.Rows.Count == 1)
                {
                    NavButtonState(false, false, false, false);
                }
                //Added by Priyanka B on 18/05/2017 End
                //NavButtonState(true, true, true, true);  //Commented by Priyanka B on 18/05/2017
            }
            ...
            else
            {
                ...
                if (md == Mode.Add)
                {
                    NavButtonState(false, false, false, false);  //Commented by Priyanka B on 18/05/2017
                }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                //string sqlstr = "update QC_PROCESS_Master set qc_sample_qty=" 
                //+ txtSampleQty.Text.Trim() + ", fail_if_qty=" + txtFailIf.Text.Trim() + "where QC_PROCESS_ID=" + dsGloble.Tables["QC_Process_Master"].Rows[CurrentProcessId]["QC_PROCESS_ID"];  //Commented by Priyanka B on 18/05/2017
                string sqlstr = "update QC_PROCESS_Master set qc_sample_qty=" + txtSampleQty.Text.Trim() 
                    + ", fail_if_qty=" + txtFailIf.Text.Trim() + "where QC_PROCESS_ID=" 
                    + dsGloble.Tables["QC_Process_Master"].Rows[0]["QC_PROCESS_ID"];  //Added by Priyanka B on 18/05/2017
                DataLayer.ExecuteSQLnonQuery(sqlstr);
                //id = Convert.ToInt32(dsGloble.Tables["QC_Process_Master"].Rows[CurrentProcessId]["QC_PROCESS_ID"]); //Commented by Priyanka B on 18/05/2017
                id = Convert.ToInt32(dsGloble.Tables["QC_Process_Master"].Rows[0]["QC_PROCESS_ID"]); //Added by Priyanka B on 18/05/2017
        ...
        private void btnBack_Click(object sender, EventArgs e)
        {
            ...
            //Commented by Priyanka B on 18/05/2017 Start
            //if (dsGloble.Tables["QC_Process_Master"].Rows.Count != 0)
            //{
            //NavButtonState(true, true, true, true);
            //Databind(0);
            //}
            //Commented by Priyanka B on 18/05/2017 End
...
//                     dgvParam.CurrentCell.OwningRow.IsNewRow 
    ...
    //                 }
        ...
        private void dgvParam_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            //<--- Added By Pankaj On 24/06/2014 for Bug-22827 Start
            dgvParam.EndEdit();
            if (dgvParam.Rows[e.RowIndex].Cells[0].Value.ToString() != "")
            {
