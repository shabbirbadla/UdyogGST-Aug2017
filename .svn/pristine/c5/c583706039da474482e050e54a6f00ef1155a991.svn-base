...
Begin
    ...
    Declare @Fld_Nm varchar(60),@SqlCommand nVarChar(4000),@FldVal NUMERIC(17,2),@ParmDefinition nvarchar(500),@sDate smalldatetime
		...
		--    end
	print @SectionList
	Select EmployeeCode=@EmployeeCode,Pay_Year=@Pay_Year,ExRec=cast(0 as int),MaxLimit=cast(0 as Decimal(12,3)),Amount=cast(0 as Decimal(12,3)),a.Section,a.DeclarationDet,a.Fld_Nm,mSortOrd=a.SortOrd,b.SortOrd into #EmpInvDec From Emp_Payroll_Declaration_Master a inner join Emp_Payroll_Section_Master b on (a.Section=b.Section) where 1=2
    ...
    SET @SqlCommand='insert into #EmpInvDec Select EmployeeCode='+Char(39)+rtrim(@EmployeeCode)+Char(39)+',Pay_Year='+Char(39)+rtrim(@Pay_Year)+Char(39)+',ExRec=cast(0 as int),a.MaxLimit,Amount=cast(0 as Decimal(12,3)),a.Section,a.DeclarationDet,a.Fld_Nm,mSortOrd=a.SortOrd,b.SortOrd From Emp_Payroll_Declaration_Master a inner join Emp_Payroll_Section_Master b on (a.Section=b.Section) where a.IntField=0 and a.Section in ('+@SectionList+') and (a.IsDeactive=0  or (a.IsDeactive=1 and a.DeactFrom >'+char(39)+cast(getdate() as varchar)+char(39)+'))order by b.SortOrd,a.SortOrd' 
   -- SET @SqlCommand='insert into #EmpInvDec Select EmployeeCode='+Char(39)+rtrim(@EmployeeCode)+Char(39)+',Pay_Year='+Char(39)+rtrim(@Pay_Year)+Char(39)+',ExRec=cast(0 as int),Amount=cast(0 as Decimal(12,3)),a.Section,a.DeclarationDet,a.Fld_Nm,mSortOrd=a.SortOrd,b.SortOrd From Emp_Payroll_Declaration_Master a inner join Emp_Payroll_Section_Master b on (a.Section=b.Section) where a.Section in ('+@SectionList+') and (a.IsDeactive=0  or (a.IsDeactive=1 and a.DeactFrom >'+char(39)+cast(getdate() as varchar)+char(39)+'))order by b.SortOrd,a.SortOrd' 
	print @SqlCommand
	execute Sp_ExecuteSql @SqlCommand
	SET @SqlCommand='Declare cur_InvDec cursor for Select distinct Fld_Nm From Emp_Payroll_Declaration_Master where IntField=0 and Section in ('+@SectionList+')'
	print @SqlCommand
	execute Sp_ExecuteSql @SqlCommand
	open cur_InvDec
