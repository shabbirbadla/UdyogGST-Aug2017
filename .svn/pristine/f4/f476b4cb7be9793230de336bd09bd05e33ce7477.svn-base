If Exists(Select [name] From SysObjects Where xtype='P' and [Name]='USP_REP_AP_VAT_200G')
Begin
	Drop Procedure USP_REP_AP_VAT_200G
End
GO
-- =============================================
-- Author:		Hetal L. Patel
-- Create date: 01/06/2009	
-- Description:	This Stored procedure is useful to generate AP VAT FORM 200G.
-- Modify date: 24/06/2009
-- Modified By: Dinesh 
-- Modified By: Changes has been done by Gaurav Tanna on 07/10/2014 for BUG-23933
-- Remark:
-- =============================================
CREATE Procedure [dbo].[USP_REP_AP_VAT_200G]
...
alter table #FORM200_G add recno int identity
*/
-- bug-23933 - end here
...
IF Exists(Select A.ID From SysObjects A Inner Join SysColumns B On(A.ID = B.ID) Where A.[Name] = 'STMAIN' And B.[Name] = 'DBNAME')
	Begin	------Fetch Records from Multi Co. Data
		...
		-- bug-23933 - start here
		--SET @SQLCOMMAND='Insert InTo #FORM200_G Select * from '+@MCON
...
-- bug-23933 - start here
/*
SELECT @AMTA1=SUM(NET_AMT)-Sum(TaxAmt) FROM #FORM200_G WHERE (DATE BETWEEN @SDATE AND @EDATE) and st_type = 'Out of Country' and BHENT = 'ST'
SELECT @AMTA2=SUM(NET_AMT)-Sum(TaxAmt) FROM #FORM200_G WHERE (st_type = 'Out of State') AND BHENT='ST' AND (DATE BETWEEN @SDATE AND @EDATE)
SELECT @AMTB1=SUM(NET_AMT)-Sum(TaxAmt), @Per = Per FROM #FORM200_G WHERE St_Type = 'Local' and (BHENT='ST') and Per = 4 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SELECT @AMTB2=SUM(NET_AMT)-Sum(TaxAmt), @Per = Per FROM #FORM200_G WHERE St_Type = 'Local' and (BHENT='ST') and Per = 12.50 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SELECT @AMTC1=SUM(NET_AMT)-Sum(TaxAmt), @Per = Per FROM #FORM200_G WHERE St_Type = 'Local' and (BHENT='ST') and Per = 1 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
*/
...
Set @AMTA1 = 0
-- bug-23933 - start here
--SELECT @AMTA1=SUM(NET_AMT)-Sum(TaxAmt) FROM #FORM200_G  WHERE (st_type = 'Out of Country') AND BHENT='ST' AND (DATE BETWEEN @SDATE AND @EDATE)
...
Set @AMTA2 = 0
-- bug-23933 - start here
--SELECT @AMTA1=SUM(NET_AMT)-Sum(TaxAmt) FROM #FORM200_G WHERE (DATE BETWEEN @SDATE AND @EDATE) and Tax_Name = 'Exempted' and BHENT = 'ST'
...
--INSERT INTO #APVAT 
--(Part,SrNo1,Taxable_Amt,taxamt1,taxamt2,taxamt3,taxamt4,tax_name,level1,st_type) VALUES (1,'D',0,0,0,0,0,'',0,'')
--INSERT INTO #APVAT 
