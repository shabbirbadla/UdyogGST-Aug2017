If Exists(Select [name] From SysObjects Where xtype='P' and [Name]='USP_REP_TG_VAT_200A')
Begin
	Drop Procedure USP_REP_TG_VAT_200A
End
GO
/*
EXECUTE USP_REP_TG_VAT_200A'','','','04/01/2013','03/31/2016','','','','',0,0,'','','','','','','','','2015-2016',''
*/
-- =============================================
-- Author:		Guarav Tanna
-- Create date: 10/10/2014	for BUG-23935 (Initially created same as AP VAT)
-- Description:	This Stored procedure is useful to generate TG VAT FORM 200.
-- Remark:
-- Modify by  : Sumit Gavate
-- Remark	  : Modify for Bug no - 27971
-- Modify date: 19/04/2015
-- =============================================
CREATE Procedure [dbo].[USP_REP_TG_VAT_200A]
...
alter table #FORM200_A add recno int identity
*/
-- bug-23933 - end here
...
IF Exists(Select A.ID From SysObjects A Inner Join SysColumns B On(A.ID = B.ID) Where A.[Name] = 'STMAIN' And B.[Name] = 'DBNAME')
	Begin	------Fetch Records from Multi Co. Data
		...
		-- bug-23933 - start here 
		--SET @SQLCOMMAND='Insert InTo #FORM200_A Select * from '+@MCON
...
--Declare @AMTD1 As Numeric(12,2),@AMTD2 As Numeric(12,2),@AMTE1 As Numeric(12,2),@AMTE2 As Numeric(12,2),
--	    @AMTF1 As Numeric(12,2),@AMTF2 As Numeric(12,2)

Select @AMTA1=0,@AMTA2=0,@AMTB1=0,@AMTB2=0,@AMTC1=0,@AMTC2=0,@per=0
Select @AMTD1=0,@AMTD2=0,@AMTE1=0,@AMTE2=0,@AMTF1=0,@AMTF2=0

---Total of 13A,14A,16A,17A,19A of VAT Form 200
-- bug-23933 - start here 
/*
SELECT @AMTA1=SUM(NET_AMT)-Sum(TaxAmt) FROM #FORM200_A WHERE (DATE BETWEEN @SDATE AND @EDATE) and st_type = 'Out of Country' and BHENT = 'ST'
SELECT @AMTA2=SUM(NET_AMT)-Sum(TaxAmt) FROM #FORM200_A WHERE (st_type = 'Out of State') AND BHENT='ST' AND (DATE BETWEEN @SDATE AND @EDATE)
SELECT @AMTB1=SUM(NET_AMT)-Sum(TaxAmt), @Per = Per FROM #FORM200_A WHERE St_Type = 'Local' and (BHENT='ST') and Per = 4 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SELECT @AMTB2=SUM(NET_AMT)-Sum(TaxAmt), @Per = Per FROM #FORM200_A WHERE St_Type = 'Local' and (BHENT='ST') and Per = 12.50 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SELECT @AMTC1=SUM(NET_AMT)-Sum(TaxAmt), @Per = Per FROM #FORM200_A WHERE St_Type = 'Local' and (BHENT='ST') and Per = 1 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
*/
SELECT @AMTA1 = ISNULL(SUM(GRO_AMT)-Sum(TaxAmt),0) FROM vattbl WHERE (DATE BETWEEN @SDATE AND @EDATE) and
	st_type = 'Out of Country' and BHENT = 'ST' and Per = 0
SELECT @AMTA2 = ISNULL(SUM(GRO_AMT)-Sum(TaxAmt),0) FROM vattbl WHERE (st_type = 'Out of State')
	AND BHENT='ST' AND (DATE BETWEEN @SDATE AND @EDATE) and Per = 0
SELECT @AMTB1 = ISNULL(SUM(GRO_AMT)-Sum(TaxAmt),0), @Per = Per FROM vattbl WHERE St_Type = 'Local'
	and (BHENT In ('ST', 'PR', 'DN')) and Per = 4 AND Tax_Name like '%VAT%' and  (DATE BETWEEN @SDATE AND @EDATE) group by Per
SELECT @AMTB2 = ISNULL(SUM(GRO_AMT)-Sum(TaxAmt),0), @Per = Per FROM vattbl WHERE St_Type = 'Local'
	and (BHENT In ('ST', 'PR', 'DN')) and Per = 12.50 and Tax_Name like '%VAT%' AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SELECT @AMTC1 = ISNULL(SUM(GRO_AMT)-Sum(TaxAmt),0), @Per = Per FROM vattbl WHERE St_Type = 'Local'
	and (BHENT In ('ST', 'PR', 'DN')) and Per = 1 and Tax_Name like '%VAT%'  AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
...
Set @AMTA1 = 0
-- bug-23933 - start here 
--SELECT @AMTA1=SUM(NET_AMT)-Sum(TaxAmt) FROM #FORM200_A  WHERE (st_type = 'Out of Country') AND BHENT='ST' AND (DATE BETWEEN @SDATE AND @EDATE)
...
INNER JOIN LCODE ON (LCODE.ENTRY_TY=STITEM.ENTRY_TY) WHERE IT_MAST.U_GVTYPE like 'Tax Free Sch - I goods' AND STMAIN.u_imporm <> 'Exempt Transaction' 
AND (STITEM.DATE BETWEEN @SDATE AND @EDATE)
--SET @AMTA1=CASE WHEN @AMTA1 IS NULL THEN 0 ELSE @AMTA1 END
...
-- bug-23933 - start here 
--SELECT @AMTA1=SUM(NET_AMT)-Sum(TaxAmt) FROM #FORM200_A WHERE (DATE BETWEEN @SDATE AND @EDATE) and Tax_Name = 'Exempted' and BHENT = 'ST'
...
INSERT INTO #APVAT 
(Part,SrNo1,Taxable_Amt,taxamt1,taxamt2,taxamt3,taxamt4,tax_name,level1,st_type) VALUES (1,'C',ISNULL(@AMTA1,0),0,0,0,0,'',0,'')
--<- PART1
...
Set @AMTA1 = 0
Set @PER = 1
SELECT @AMTA1=SUM(TAXAMT),@Per=Per FROM #FORM200_A WHERE (BHENT='PT') and St_Type = 'Local' and Per = 1 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SET @AMTA1=CASE WHEN @AMTA1 IS NULL THEN 0 ELSE @AMTA1 END
...
Set @AMTA1 = 0
Set @PER = 4
SELECT @AMTA1=SUM(TAXAMT),@PER=Per FROM #FORM200_A WHERE (BHENT='PT') and St_Type = 'Local'  and Per = 4 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SET @AMTA1=CASE WHEN @AMTA1 IS NULL THEN 0 ELSE @AMTA1 END
INSERT INTO #APVAT 
...
Set @AMTA1 = 0
Set @PER = 12.5
SELECT @AMTA1=SUM(TAXAMT),@Per=Per FROM #FORM200_A WHERE (BHENT='PT') and St_Type = 'Local'  and Per = 12.5 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SET @AMTA1=CASE WHEN @AMTA1 IS NULL THEN 0 ELSE @AMTA1 END
INSERT INTO #APVAT 
...
*/
 ...
 BEGIN
	...
	Begin
		-- bug-23933 - start here 
		/*
		SELECT @AMTA1=Round(SUM(Net_AMT),0)  FROM (select distinct tran_cd,bhent,net_amt,dbname from #FORM200_A WHERE ST_TYPE='LOCAL' AND BHENT='PT' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And S_tax <> '') b
		SELECT @AMTB1=Round(SUM(TAXAMT),0)   FROM #FORM200_A WHERE ST_TYPE='LOCAL' AND BHENT='PT' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And S_tax <> '' 
		SELECT @AMTC1=Round(SUM(VATONAMT),0) FROM (select distinct tran_cd,bhent,vatonamt,dbname from #FORM200_A WHERE ST_TYPE='LOCAL' AND BHENT='PR' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And S_tax <> '') b
		SELECT @AMTD1=Round(SUM(TAXAMT),0)   FROM #FORM200_A WHERE ST_TYPE='LOCAL' AND BHENT='PR' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And S_tax <> '' 
		SELECT @AMTF1=Round(SUM(VATONAMT),0) FROM (select distinct tran_cd,bhent,vatonamt,dbname from #FORM200_A WHERE ST_TYPE='LOCAL' AND BHENT='EP' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And S_tax <> '') b
		SELECT @AMTF2=Round(SUM(TAXAMT),0)   FROM #FORM200_A WHERE ST_TYPE='LOCAL' AND BHENT='EP' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And S_tax <> '' 
		SELECT @AMTG1=Round(SUM(VATONAMT),0) FROM (select distinct tran_cd,bhent,vatonamt,dbname from #FORM200_A WHERE ST_TYPE='LOCAL' AND BHENT='DN' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And S_tax <> '') b
		SELECT @AMTG2=Round(SUM(TAXAMT),0)   FROM #FORM200_A WHERE ST_TYPE='LOCAL' AND BHENT='DN' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And S_tax <> '' 
		SELECT @AMTH1=Round(SUM(NET_AMT),0)  FROM (select distinct tran_cd,bhent,net_amt,dbname from #FORM200_A WHERE ST_TYPE='LOCAL' AND BHENT='ST' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And S_tax <> '' AND U_IMPORM = 'Purchase Return') b
		SELECT @AMTH2=Round(SUM(TAXAMT),0)   FROM #FORM200_A WHERE ST_TYPE='LOCAL' AND BHENT='ST' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And S_tax <> '' AND U_IMPORM = 'Purchase Return' 
		*/
