...
IF VARTYPE(uexapps) <> 'C' OR EMPTY(uexapps)
	=MESSAGEBOX('In Configuration file Xfile Path cannot be empty',16,'Udyog Admin')
	RETURN .F.
ELSE
	IF !FILE(uexapps)
		=MESSAGEBOX('In Configuration file Specify application file is not found',16,'Udyog Admin')
		RETURN .F.
	ENDIF
	lcExeName = ALLT(uexapps)
	DO &lcExeName WITH [ST3]
ENDIF
