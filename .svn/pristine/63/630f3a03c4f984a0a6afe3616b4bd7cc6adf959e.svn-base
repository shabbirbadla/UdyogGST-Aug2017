IF EXISTS(SELECT XTYPE,NAME FROM SYSOBJECTS WHERE XTYPE='P' AND name='USP_REP_RJ_VAT07')
BEGIN
	DROP PROCEDURE USP_REP_RJ_VAT07
END
SET ANSI_NULLS ON
GO
...
-- Description:	This Stored procedure is useful to Generate Rajasthan VAT FORM 07
-- Modify date: 
-- Modified By: 
-- Modify date: 
-- Remark:    : 
-- EXECUTE USP_REP_RJ_VAT07 '','','','04/01/2014','03/31/2015','','','','',0,0,'','','','','','','','','2014-2015',''
-- ============================================= 


CREATE PROCEDURE [dbo].[USP_REP_RJ_VAT07]
...
SELECT PART=3,PARTSR='AAA',SRNO='AAA',a.inv_no,m.date,AMT1=NET_AMT,AMT2=M.TAXAMT,Schedule=space(100),VATONAMT=net_amt,
...
SET @CHAR=65
declare @inv_no as varchar(10),@DATE as smalldatetime ,@Party_nm as varchar(50),@S_TAX as varchar(30),@Schedule as varchar(30),@Amt1 as numeric(18,4),@Amt2 as numeric(18,4),@VATonamt as numeric(18,4)
Select @inv_no='',@DATE='',@Party_nm='',@S_TAX='',@Schedule='',@AMT1=0, @AMT2=0 ,@VATONAMT=0

Declare CUR_FORMRJ07Aaaa cursor for

SELECT inv_no=a.inv_no,date=a.date,party_nm=a.ac_name,s_tax=a.s_tax,schedule=it.U_SHCODE,AMTA1=ISNULL(sum(A.GRO_AMT),0),AMT2=SUM(A.TAXAMT),
VATONAMT=a.vatonamt
FROM VATTBL A
inner join  it_mast it on (it.it_code=a.it_code)
WHERE  A.BHENT='PT' AND (A.DATE BETWEEN @SDATE AND @EDATE) and a.st_type in('Local','') and a.itemtype='I'
group by a.inv_no,a.ac_name,a.s_tax,it.u_shcode,a.date,a.taxamt,a.gro_amt,a.vatonamt


OPEN CUR_FORMRJ07Aaaa
FETCH NEXT FROM CUR_FORMRJ07Aaaa INTO @inv_no,@DATE,@Party_nm,@S_TAX,@Schedule,@AMT1,@AMT2,@VATONAMT
...
BEGIN
    ...
    INSERT INTO #FORMRJ07A (PART,PARTSR,SRNO,INV_NO,DATE,PARTY_NM,S_TAX,Schedule,AMT1,AMT2,VATONAMT) VALUES(1,'1',CHAR(@CHAR),@INV_NO,@DATE,@PARTY_NM,@S_TAX,@Schedule,@AMT1,@AMT2,@VATONAMT)
...
FROM VATTBL A
inner join  it_mast it on (it.it_code=a.it_code)
WHERE  A.BHENT='PT' AND (A.DATE BETWEEN @SDATE AND @EDATE) and a.st_type in('Local','') and a.itemtype='C'
group by a.inv_no,a.ac_name,a.s_tax,a.date,a.taxamt,a.gro_amt,it.u_shcode,a.vatonamt
...
BEGIN
    ...
    INSERT INTO #FORMRJ07A (PART,PARTSR,SRNO,INV_NO,DATE,PARTY_NM,S_TAX,Schedule,AMT1,AMT2,VATONAMT) VALUES(1,'2',CHAR(@CHAR),@INV_NO,@DATE,@PARTY_NM,@S_TAX,@Schedule,@AMT1,@AMT2,@VATONAMT)
...
IF @nCount = 0
	INSERT INTO #FORMRJ07A
	    (PART,PARTSR,SRNO,INV_NO,DATE,PARTY_NM,S_TAX,Schedule,AMT1,AMT2,VATONAMT)
	    VALUES
	    (1,'1',        '',  ''  ,'',    '',      '',    '' ,   0,   0,0)
...
Update #FORMRJ07A set  PART = isnull(Part,0) , Partsr = isnull(PARTSR,''), SRNO = isnull(SRNO,''),
		               ...
		               Schedule = isnull(Schedule,''),  AMT1 = isnull(AMT1,0), AMT2 = isnull(AMT2,0)  ,VATONAMT=isnull(VATONAMT,0)     
