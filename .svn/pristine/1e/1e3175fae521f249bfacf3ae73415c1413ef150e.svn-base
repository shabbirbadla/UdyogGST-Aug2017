If Exists(Select [name] From SysObjects Where xtype='P' and [Name]='USP_REP_TG_VAT_200D')
Begin
	Drop Procedure USP_REP_TG_VAT_200D
End
GO
-- =============================================
-- Author:		Guarav Tanna
-- Create date: 10/10/2014	for BUG-23935 (Initially created same as AP VAT)
-- Description:	This Stored procedure is useful to generate TG VAT FORM 200D.
-- Remark:
-- Modify by  : Sumit Gavate
-- Remark	  : Modify for Bug no - 27971
-- Modify date: 19/04/2015
-- =============================================
CREATE Procedure [dbo].[USP_REP_TG_VAT_200D]
...
alter table #FORM200_D add recno int identity
*/
-- bug-23933 - end here
...
IF Exists(Select A.ID From SysObjects A Inner Join SysColumns B On(A.ID = B.ID) Where A.[Name] = 'STMAIN' And B.[Name] = 'DBNAME')
	Begin	------Fetch Records from Multi Co. Data
		...
		-- bug-23933 - start here 
		--SET @SQLCOMMAND='Insert InTo #FORM200_D Select * from '+@MCON
...
-- bug-23933 - start here 
/*
SELECT @AMTA1=SUM(NET_AMT)-Sum(TaxAmt) FROM #FORM200_D WHERE (DATE BETWEEN @SDATE AND @EDATE) and st_type = 'Out of Country' and BHENT = 'ST'
SELECT @AMTA2=SUM(NET_AMT)-Sum(TaxAmt) FROM #FORM200_D WHERE (st_type = 'Out of State') AND BHENT='ST' AND (DATE BETWEEN @SDATE AND @EDATE)
SELECT @AMTB1=SUM(NET_AMT)-Sum(TaxAmt), @Per = Per FROM #FORM200_D WHERE St_Type = 'Local' and (BHENT='ST') and Per = 4 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SELECT @AMTB2=SUM(NET_AMT)-Sum(TaxAmt), @Per = Per FROM #FORM200_D WHERE St_Type = 'Local' and (BHENT='ST') and Per = 12.50 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SELECT @AMTC1=SUM(NET_AMT)-Sum(TaxAmt), @Per = Per FROM #FORM200_D WHERE St_Type = 'Local' and (BHENT='ST') and Per = 1 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
*/
...
Set @AMTA1 = 0
-- bug-23933 - start here 
--SELECT @AMTA1=SUM(NET_AMT)-Sum(TaxAmt) FROM #FORM200_D  WHERE (st_type = 'Out of Country') AND BHENT='ST' AND (DATE BETWEEN @SDATE AND @EDATE)
...
Set @AMTA2 = 0
-- bug-23933 - start here 
--SELECT @AMTA1=SUM(NET_AMT)-Sum(TaxAmt) FROM #FORM200_D WHERE (DATE BETWEEN @SDATE AND @EDATE) and Tax_Name = 'Exempted' and BHENT = 'ST'
...
INSERT INTO #APVAT (Part,SrNo1,Taxable_Amt,taxamt1,taxamt2,taxamt3,taxamt4,tax_name,level1,st_type) VALUES (1,'C',@AMTA1,0,0,0,0,'',0,'')
--<- PART1
...
Set @AMTA1 = 0
Set @PER = 1
SELECT @AMTA1=SUM(TAXAMT),@Per=Per FROM #FORM200_D WHERE (BHENT='PT') and St_Type = 'Local' and Per = 1 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SET @AMTA1=CASE WHEN @AMTA1 IS NULL THEN 0 ELSE @AMTA1 END
...
Set @AMTA1 = 0
Set @PER = 4
SELECT @AMTA1=SUM(TAXAMT),@PER=Per FROM #FORM200_D WHERE (BHENT='PT') and St_Type = 'Local'  and Per = 4 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SET @AMTA1=CASE WHEN @AMTA1 IS NULL THEN 0 ELSE @AMTA1 END
INSERT INTO #APVAT 
...
Set @AMTA1 = 0
Set @PER = 12.5
SELECT @AMTA1=SUM(TAXAMT),@Per=Per FROM #FORM200_D WHERE (BHENT='PT') and St_Type = 'Local'  and Per = 12.5 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SET @AMTA1=CASE WHEN @AMTA1 IS NULL THEN 0 ELSE @AMTA1 END
INSERT INTO #APVAT 
...
--<- PART2
 ...
 BEGIN
  ...
  -- bug-23933 - (instead of FROM #FORM200_D, changed it to FROM VATTBL)
	Begin
