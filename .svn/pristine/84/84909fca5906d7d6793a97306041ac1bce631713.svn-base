If Exists(Select [name] From SysObjects Where xtype='P' and [Name]='USP_REP_AP_VAT_200A')
Begin
	Drop Procedure USP_REP_AP_VAT_200A
End
GO
-- =============================================
-- Author:		Hetal L. Patel
-- Create date: 01/06/2009	
-- Description:	This Stored procedure is useful to generate AP VAT FORM 200A.
...
-- Remark:
-- =============================================
CREATE Procedure [dbo].[USP_REP_AP_VAT_200A]
...
SELECT @AMTB1=SUM(GRO_AMT)-Sum(TaxAmt), @Per = Per FROM vattbl WHERE St_Type = 'Local' and (BHENT In ('ST', 'PR', 'DN')) and Per = 4 AND Tax_Name like '%VAT%' and  (DATE BETWEEN @SDATE AND @EDATE) group by Per
SELECT @AMTB2=SUM(GRO_AMT)-Sum(TaxAmt), @Per = Per FROM vattbl WHERE St_Type = 'Local' and (BHENT In ('ST', 'PR', 'DN')) and Per = 12.50 and Tax_Name like '%VAT%' AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SELECT @AMTC1=SUM(GRO_AMT)-Sum(TaxAmt), @Per = Per FROM vattbl WHERE St_Type = 'Local' and (BHENT In ('ST', 'PR', 'DN')) and Per = 1 and Tax_Name like '%VAT%'  AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
