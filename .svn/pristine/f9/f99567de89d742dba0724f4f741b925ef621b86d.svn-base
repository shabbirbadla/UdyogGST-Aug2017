If Exists(Select [name] From SysObjects Where xtype='P' and [Name]='USP_REP_TG_VAT_200B')
Begin
	Drop Procedure USP_REP_TG_VAT_200B
End
GO
/*
EXECUTE USP_REP_TG_VAT_200B'','','','04/01/2013','03/31/2016','','','','',0,0,'','','','','','','','','2015-2016',''
*/
-- =============================================
-- Author:		Guarav Tanna
-- Create date: 10/10/2014	for BUG-23935 (Initially created same as AP VAT)
-- Description:	This Stored procedure is useful to generate TG VAT FORM 200B.
-- Remark:
-- Modify by  : Sumit Gavate
-- Remark	  : Modify for Bug no - 27971
-- Modify date: 19/04/2015
-- =============================================
CREATE Procedure [dbo].[USP_REP_TG_VAT_200B]
...
alter table #FORM200_B add recno int identity
*/
-- bug-23933 - end here
...
IF Exists(Select A.ID From SysObjects A Inner Join SysColumns B On(A.ID = B.ID) Where A.[Name] = 'STMAIN' And B.[Name] = 'DBNAME')
	Begin	------Fetch Records from Multi Co. Data
		...
		-- bug-23933 - start here 
		--SET @SQLCOMMAND='Insert InTo  #FORM200_B Select * from '+@MCON		
...
-- bug-23933 - start here 
/*
SELECT @AMTA1=SUM(NET_AMT)-Sum(TaxAmt) FROM #FORM200_B WHERE (DATE BETWEEN @SDATE AND @EDATE) and st_type = 'Out of Country' and BHENT = 'ST'
SELECT @AMTA2=SUM(NET_AMT)-Sum(TaxAmt) FROM #FORM200_B WHERE (st_type = 'Out of State') AND BHENT='ST' AND (DATE BETWEEN @SDATE AND @EDATE)
SELECT @AMTB1=SUM(NET_AMT)-Sum(TaxAmt), @Per = Per FROM #FORM200_B WHERE St_Type = 'Local' and (BHENT='ST') and Per = 4 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SELECT @AMTB2=SUM(NET_AMT)-Sum(TaxAmt), @Per = Per FROM #FORM200_B WHERE St_Type = 'Local' and (BHENT='ST') and Per = 12.50 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SELECT @AMTC1=SUM(NET_AMT)-Sum(TaxAmt), @Per = Per FROM #FORM200_B WHERE St_Type = 'Local' and (BHENT='ST') and Per = 1 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
*/
SELECT @AMTA1 = ISNULL(SUM(GRO_AMT)-Sum(TaxAmt),0) FROM VATTBL WHERE (DATE BETWEEN @SDATE AND @EDATE) and st_type = 'Out of Country' and BHENT = 'ST' and Per = 0
SELECT @AMTA2 = ISNULL(SUM(GRO_AMT)-Sum(TaxAmt),0) FROM VATTBL WHERE (st_type = 'Out of State') AND BHENT='ST' AND (DATE BETWEEN @SDATE AND @EDATE) and Per = 0
SELECT @AMTB1 = ISNULL(SUM(GRO_AMT)-Sum(TaxAmt),0), @Per = Per FROM VATTBL WHERE St_Type = 'Local' and Tax_Name like '%VAT%' and (BHENT In ('ST', 'PR', 'DN')) and Per = 4 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SELECT @AMTB2 = ISNULL(SUM(GRO_AMT)-Sum(TaxAmt),0), @Per = Per FROM VATTBL WHERE St_Type = 'Local' and Tax_Name like '%VAT%' and (BHENT In ('ST', 'PR', 'DN')) and Per = 12.50 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SELECT @AMTC1 = ISNULL(SUM(GRO_AMT)-Sum(TaxAmt),0), @Per = Per FROM VATTBL WHERE St_Type = 'Local' and Tax_Name like '%VAT%' and (BHENT In ('ST', 'PR', 'DN')) and Per = 1 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
-- bug-23933 - end here 
...
Set @AMTA1 = 0
-- bug-23933 - start here 
--SELECT @AMTA1=SUM(NET_AMT)-Sum(TaxAmt) FROM #FORM200_B  WHERE (st_type = 'Out of Country') AND BHENT='ST' AND (DATE BETWEEN @SDATE AND @EDATE)

--SELECT @AMTA1=SUM(GRO_AMT)-Sum(TaxAmt) FROM VATTBL  WHERE (st_type = 'Out of Country') AND BHENT='ST' AND (DATE BETWEEN @SDATE AND @EDATE)
SELECT @AMTA1= CASE WHEN LCODE.STAX_ITEM = 1 THEN (round(STITEM.u_asseamt+STITEM.EXAMT+STITEM.U_CESSAMT+STITEM.U_HCESAMT+STITEM.tot_add+STITEM.TOT_DEDUC,2))
else round(((STITEM.gro_amt+STITEM.tot_add+STITEM.tot_tax)-STITEM.tot_deduc),2) end FROM STITEM INNER JOIN IT_MAST ON (STITEM.It_code = IT_MAST.It_code)
INNER JOIN LCODE ON (LCODE.ENTRY_TY=STITEM.ENTRY_TY) INNER JOIN STMAIN ON (STITEM.inv_no = STMAIN.inv_no AND STITEM.Tran_cd = STMAIN.Tran_cd AND STITEM.date = STMAIN.date) 
...
--SET @AMTA1=CASE WHEN @AMTA1 IS NULL THEN 0 ELSE @AMTA1 END
INSERT INTO #APVAT (Part,SrNo1,Taxable_Amt,taxamt1,taxamt2,taxamt3,taxamt4,tax_name,level1,st_type) VALUES (1,'B',ISNULL(@AMTA1,0),0,0,0,0,'',0,'')
...
Set @AMTA2 = 0
-- bug-23933 - start here 
--SELECT @AMTA1=SUM(NET_AMT)-Sum(TaxAmt) FROM #FORM200_B WHERE (DATE BETWEEN @SDATE AND @EDATE) and Tax_Name = 'Exempted' and BHENT = 'ST'

--SELECT @AMTA1=SUM(GRO_AMT)-Sum(TaxAmt) FROM VATTBL WHERE (DATE BETWEEN @SDATE AND @EDATE) and Tax_Name = 'Exempted' and BHENT = 'ST'
...
Set @AMTA1 = 0
Set @PER = 1
SELECT @AMTA1=SUM(TAXAMT),@Per=Per FROM #FORM200_B WHERE (BHENT='PT') and St_Type = 'Local' and Per = 1 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SET @AMTA1=CASE WHEN @AMTA1 IS NULL THEN 0 ELSE @AMTA1 END
...
Set @AMTA1 = 0
Set @PER = 4
SELECT @AMTA1=SUM(TAXAMT),@PER=Per FROM #FORM200_B WHERE (BHENT='PT') and St_Type = 'Local'  and Per = 4 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SET @AMTA1=CASE WHEN @AMTA1 IS NULL THEN 0 ELSE @AMTA1 END
INSERT INTO #APVAT 
...
Set @AMTA1 = 0
Set @PER = 12.5
SELECT @AMTA1=SUM(TAXAMT),@Per=Per FROM #FORM200_B WHERE (BHENT='PT') and St_Type = 'Local'  and Per = 12.5 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SET @AMTA1=CASE WHEN @AMTA1 IS NULL THEN 0 ELSE @AMTA1 END
INSERT INTO #APVAT 
(Part,SrNo1,Taxable_Amt,taxamt1,taxamt2,taxamt3,taxamt4,tax_name,level1,st_type) VALUES (2,'C',0,0,0,@AMTA1,0,'',@PER,'')
*/
 SELECT @AMTA1=0,@AMTB1=0,@AMTC1=0,@AMTD1=0,@AMTE1=0,@AMTF1=0,@AMTF2=0,@AMTG1=0,@AMTG2=0,@AMTH1=0,@AMTI1=0,@AMTJ1=0,@AMTK1=0,@AMTL1=0,@AMTM1=0,@AMTN1=0,@AMTO1=0 
 ...
 BEGIN
    ...
    -- bug-23933 - (instead of FROM #FORM200_B, changed it to FROM VATTBL)  
	Begin
...
SET @Per  =CASE WHEN @PER IS NULL THEN 0 ELSE @PER END
INSERT INTO #APVAT 
(Part,SrNo1,Taxable_Amt,taxamt1,taxamt2,taxamt3,taxamt4,tax_name,level1,st_type) VALUES (3,'A',0,@AMTA1,0,0,0,'',@PER,'')
...
SET @AMTA1=CASE WHEN @AMTA1 IS NULL THEN 0 ELSE @AMTA1 END
INSERT INTO #APVAT 
(Part,SrNo1,Taxable_Amt,taxamt1,taxamt2,taxamt3,taxamt4,tax_name,level1,st_type) VALUES (3,'B',0,@AMTA1,0,0,0,'',@PER,'')
...
SET @AMTA1=CASE WHEN @AMTA1 IS NULL THEN 0 ELSE @AMTA1 END
INSERT INTO #APVAT 
(Part,SrNo1,Taxable_Amt,taxamt1,taxamt2,taxamt3,taxamt4,tax_name,level1,st_type) VALUES (3,'C',0,0,0,@AMTA1,0,'',@PER,'')
*/

 SELECT @AMTA1=0,@AMTB1=0,@AMTC1=0,@AMTD1=0,@AMTE1=0,@AMTF1=0,@AMTF2=0,@AMTG1=0,@AMTG2=0,@AMTH1=0,@AMTH2=0,@AMTI1=0,@AMTI2=0,@AMTJ1=0,@AMTJ2=0,@AMTK1=0,@AMTL1=0,@AMTM1=0,@AMTN1=0,@AMTO1=0 
 ...
 BEGIN
	...
	Begin
		SELECT @AMTA1=SUM(GRO_AMT)  FROM (select distinct tran_cd,bhent,GRO_amt,dbname from VATTBL WHERE ST_TYPE='LOCAL' AND BHENT='PT' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And Tax_name like '%VAT%' and s_tax<>'') b
		SELECT @AMTB1=SUM(TAXAMT)   FROM VATTBL WHERE ST_TYPE='LOCAL' AND BHENT='PT' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And Tax_name like '%VAT%' and s_tax<>''
		SELECT @AMTC1=SUM(GRO_AMT) FROM (select distinct tran_cd,bhent,GRO_AMT,dbname from VATTBL WHERE ST_TYPE='LOCAL' AND BHENT='PR' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And Tax_name like '%VAT%' and s_tax<>'') b
		SELECT @AMTD1=SUM(TAXAMT)   FROM VATTBL WHERE ST_TYPE='LOCAL' AND BHENT='PR' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And Tax_name like '%VAT%' and s_tax<>''
		SELECT @AMTF1=SUM(GRO_AMT) FROM (select distinct tran_cd,bhent,GRO_AMT,dbname from VATTBL WHERE ST_TYPE='LOCAL' AND BHENT='EP' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And Tax_name like '%VAT%' and s_tax<>'') b
		SELECT @AMTF2=SUM(TAXAMT)   FROM VATTBL WHERE ST_TYPE='LOCAL' AND BHENT='EP' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And Tax_name like '%VAT%' and s_tax<>''
		SELECT @AMTG1=SUM(GRO_AMT) FROM (select distinct tran_cd,bhent,GRO_AMT,dbname from VATTBL WHERE ST_TYPE='LOCAL' AND BHENT='DN' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And Tax_name like '%VAT%') b
		SELECT @AMTG2=SUM(TAXAMT)   FROM VATTBL WHERE ST_TYPE='LOCAL' AND BHENT='DN' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And Tax_name like '%VAT%' 
		SELECT @AMTH1=SUM(GRO_AMT)  FROM (select distinct tran_cd,bhent,GRO_amt,dbname from VATTBL WHERE ST_TYPE='LOCAL' AND BHENT='ST' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And Tax_name like '%VAT%') b
		SELECT @AMTH2=SUM(TAXAMT)   FROM VATTBL WHERE ST_TYPE='LOCAL' AND BHENT='ST' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And Tax_name like '%VAT%' 
		SELECT @AMTJ1=SUM(GRO_AMT)  FROM (select distinct tran_cd,bhent,GRO_amt,dbname from VATTBL WHERE ST_TYPE='LOCAL' AND BHENT='CN' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And Tax_name like '%VAT%' and s_tax<>'') b
		SELECT @AMTJ2=SUM(TAXAMT)   FROM VATTBL WHERE ST_TYPE='LOCAL' AND BHENT='CN' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And Tax_name like '%VAT%' and s_tax<>''
		SELECT @AMTI1=SUM(GRO_AMT)  FROM (select distinct tran_cd,bhent,GRO_amt,dbname from VATTBL WHERE ST_TYPE='LOCAL' AND BHENT='SR' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And Tax_name like '%VAT%' and s_tax<>'') b
		SELECT @AMTI2=SUM(TAXAMT)   FROM VATTBL WHERE ST_TYPE='LOCAL' AND BHENT='SR' AND PER=@PER AND (DATE BETWEEN @SDATE AND @EDATE) And Tax_name like '%VAT%' and s_tax<>''
	End
  ...
  SET @AMTJ2=ISNULL(@AMTJ2,0)
  --Sale Return
  SET @AMTI1=ISNULL(@AMTI1,0)
  SET @AMTI2=ISNULL(@AMTI2,0)
  ...
  if @nettax <> 0
	  Begin
		  INSERT INTO #APVAT 
          (Part,SrNo1,Taxable_Amt,taxamt1,taxamt2,taxamt3,taxamt4,tax_name,level1,st_type) 
		   VALUES (3,CHAR(@CHAR),0,@NETTAX,0,@NETEFF,0,'',@PER,'')
  ...
  */
  ...
  SET @NETTAX =  ((@AMTD1 + @AMTG2 + @AMTH2) - (@AMTB1 + @AMTF2 + @AMTJ2 + @AMTI2))
  ...
  if (@nettax <> 0) Or (@Per = 1.00 Or @Per = 4.00 Or @Per = 12.50)  
	  ...
	  Begin
			IF (@per = 4.00)		
				INSERT INTO #APVAT 
	           (Part,SrNo1,Taxable_Amt,taxamt1,taxamt2,taxamt3,taxamt4,tax_name,level1,st_type) 
			   VALUES (3,'B',0,(@AMTD1 + @AMTG2 + @AMTH2),(@AMTB1 + @AMTF2 + @AMTJ2 + @AMTI2),@NETTAX,0,'',@PER,'')
			ELSE IF (@per = 12.50)
				INSERT INTO #APVAT 
	           (Part,SrNo1,Taxable_Amt,taxamt1,taxamt2,taxamt3,taxamt4,tax_name,level1,st_type) 
			   VALUES (3,'C',0,(@AMTD1 + @AMTG2 + @AMTH2),(@AMTB1 + @AMTF2 + @AMTJ2 + @AMTI2),@NETTAX,0,'',@PER,'')
			ELSE IF (@per = 1.00)
			   INSERT INTO #APVAT 
	           (Part,SrNo1,Taxable_Amt,taxamt1,taxamt2,taxamt3,taxamt4,tax_name,level1,st_type) 
			   VALUES (3,'A',0,(@AMTD1 + @AMTG2 + @AMTH2),(@AMTB1 + @AMTF2 + @AMTJ2 + @AMTI2),@NETTAX,0,'',@PER,'')
	end
