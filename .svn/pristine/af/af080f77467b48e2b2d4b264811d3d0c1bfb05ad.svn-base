LPARAMETERS _mEraseFldrNm,_mMoveFldrNm,_mEraseFileNm,_MsgToShow
IF TYPE('_MsgToShow') != 'C'
	_MsgToShow = ''
Endif
					...
					IF !"D"$_mflist[i1,5] 
						TRY
							...
							IF DIRECTORY(_NewFldrNm,1) = .f.
								MkDir (_NewFldrNm)
								_MsgToShowVal = 'Please Copy File from '+_NewFldrNm+' Folder Manually.'
								IF !EMPTY(_MsgToShow)
									_MsgToShowVal = EVALUATE(_MsgToShow)
								Endif	
								=ErrLog(_MsgToShowVal,'W')														
							Endif		
