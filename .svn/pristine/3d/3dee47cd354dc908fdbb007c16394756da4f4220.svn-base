...
namespace eFillingExtraction
{
    class WriteTCSExcel
    {
        ...
        public WriteTCSExcel(string ExcelFileName)
        {
        ...
        public string WriteToFile(DataSet WriteDataSet)
        {
            ...
            try
            {
                ...
                MSExcel.Worksheet oSheet = (MSExcel.Worksheet)oWorkSheets.get_Item("Party");
                //for (int j = 0; j < WriteDataSet.Tables["Party"].Columns.Count; j++)
                //{
                //    MSExcel.Range CellRange = oSheet.get_Range(oSheet.Cells[2, j + 1], oSheet.Cells[WriteDataSet.Tables["Party"].Rows.Count + 1, j + 1]);
                //    switch (WriteDataSet.Tables["Party"].Columns[j].GetType().ToString())
                //    {
                //        case "System.String":
                ...
                for (int i = 1; i <= WriteDataSet.Tables["Party"].Rows.Count; i++)
                {
                    for (int j = 1; j <= WriteDataSet.Tables["Party"].Columns.Count; j++)
                    {
                        MSExcel.Range CellRange = oSheet.get_Range(oSheet.Cells[i + 1, j], oSheet.Cells[i + 1, j]);
                        switch (WriteDataSet.Tables["Party"].Rows[i - 1][j - 1].GetType().ToString())
                        {
                            case "System.String":
                                oSheet.Cells[i + 1, j] = "'" + Convert.ToString(WriteDataSet.Tables["Party"].Rows[i - 1][j - 1]);
                                CellRange.HorizontalAlignment = MSExcel.XlHAlign.xlHAlignLeft;
                                break;
                            case "System.Decimal":
                                CellRange.NumberFormat = "0.00";
                                oSheet.Cells[i + 1, j] = Convert.ToDecimal(WriteDataSet.Tables["Party"].Rows[i - 1][j - 1]);
                                CellRange.HorizontalAlignment = MSExcel.XlHAlign.xlHAlignRight;
                                break;
                            default:
                                break;
