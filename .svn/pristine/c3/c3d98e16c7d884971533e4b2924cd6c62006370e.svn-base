If Exists(Select [name] From SysObjects Where xtype='P' and [Name]='USP_MULTI_USP_REP_SALESTAX_FORM_TOBE_ISSUED')
Begin
	Drop Procedure USP_MULTI_USP_REP_SALESTAX_FORM_TOBE_ISSUED
End
GO
-- =============================================
...
-- ============================================= 
CREATE Procedure [dbo].[USP_MULTI_USP_REP_SALESTAX_FORM_TOBE_ISSUED]
...
BEGIN	
	...
	,dept=M.cate,m.cate,u_broker=M.cate
	,dbname=space(15),CompNm=SPACE(100),stax_no=SPACE(15)'+@COM_SQLSTR+'
	into ##stax_form_tobe_issued
	from stmain m 
	...
	,bcode_nm,code_nm,add1,add2,add3,contact,city,zip,state,formidt,formrdt,u_pinvno,u_pinvdt,dept,cate,u_broker'
	...
	begin
		...
		set @SQLCOMMAND =''
		SET @SQLCOMMAND='INSERT INTO ##stax_form_tobe_issued ('+@IntoStr+@SQL_TMPFLD+' )
						 Execute '+rtrim(@dbname)+'..USP_REP_SALESTAX_FORM_TOBE_ISSUED'+char(39)+@vformnm+char(39)+','+char(39)+@vparty+char(39)+','+char(39)+@mCondn+char(39)+','+cast(@vform as varchar)+','+char(39)+cast(@sdate as varchar)+char(39)+','+char(39)+cast(@edate as varchar)+char(39)+','+char(39)+@dept+char(39)+','+char(39)+@cate+char(39)+','+char(39)+@broker+char(39)
		Execute sp_executesql @sqlcommand		
		update ##stax_form_tobe_issued set dbname = rtrim(@dbname),CompNm=rtrim(@co_name),stax_no=rtrim(@stax_no) WHERE ISNULL(dbname,'')=''
		fetch next from #temp_db into @co_name,@dbname,@stax_no		
	end	
	...
	,bcode_nm,code_nm,add1,add2,add3,contact,city,zip,state,formidt,formrdt,u_pinvno,u_pinvdt,dept,cate,u_broker '+@xSQL_TMPFLD+' INTO '+@tmpTbl+' FROM   ##stax_form_tobe_issued'
	EXECUTE SP_EXECUTESQL @WSQL
	
	DROP TABLE ##stax_form_tobe_issued
