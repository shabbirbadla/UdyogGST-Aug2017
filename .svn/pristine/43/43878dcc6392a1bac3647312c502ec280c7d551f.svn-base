...
namespace eMailClient.BLL
{
    public class cls_Gen_Mgr_ProcExecute : cls_Gen_Ent_ProcExecute
    {
                        ...
                        switch (Convert.ToString(m_Obj_DR["attachment_typ"]).Trim().ToUpper())
                        {
                            case "CSV":
                                ExportCSV(Convert.ToString(m_Obj_DR["id"]).Trim(),
                                          ...
                                          DBNull.Value == m_Obj_DR["digsign"] ? false : Convert.ToBoolean(m_Obj_DR["digsign"])   // Added By pankaj For digital Signature
                                          );    
                            ...
                            case "PDF":
                                ...
                                //var directory = "D:\\Usquare"+"\\"+ getRptFilePath();//Pankaj
        ...
        private void ExportCSV(string id,
                               ...
                               bool digsign         // Added By pankaj For digital Signature
                    )       
        {
            ...
            else
            {
                ...
                foreach (DataRow m_MainDR in m_MainDs.Tables[0].Rows)
                {
                    ...
                    try
                    {
                        ...
                        m_Obj_Report.Isdigsign = digsign; 
                        if (m_MainDs.Tables[0].Columns.Contains("Dept") || m_MainDs.Tables[0].Columns.Contains("dept"))
                        {
                            m_Obj_Report.Dept = m_MainDR["Dept"].ToString().Trim();
                        }
                        if (m_MainDs.Tables[0].Columns.Contains("Cate") || m_MainDs.Tables[0].Columns.Contains("cate"))
                        {
                            m_Obj_Report.Cate = m_MainDR["Cate"].ToString().Trim();
                        }
                        if (m_MainDs.Tables[0].Columns.Contains("Inv_sr") || m_MainDs.Tables[0].Columns.Contains("inv_sr"))
                        {
                            m_Obj_Report.Inv_sr = m_MainDR["Inv_sr"].ToString().Trim();
                        }
                        m_Obj_Report.Entry_ty = m_MainDR["entry_ty"].ToString().Trim();
                        m_Obj_Report.Inv_dt = Convert.ToDateTime(m_MainDR["date"]);
                        // Added By pankaj End
                        if (InvPrint == false)      // Added by Sachin N. S. on 17/01/2014 for Bug-20211
                        {
                            m_Obj_Report.Sqlstring = reportcommand;
