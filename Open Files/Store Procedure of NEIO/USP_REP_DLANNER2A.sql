If Exists(Select [Name] from Sysobjects where xType='P' and Id=Object_Id(N'USP_REP_DLANNER2A'))
Begin
	Drop Procedure USP_REP_DLANNER2A
End
Go
...
/*
EXECUTE USP_REP_DLANNER2A'','','','04/01/2011','03/30/2015','','','','',0,0,'','','','','','','','','2010-2014',''
*/
...
-- =============================================
create PROCEDURE [dbo].[USP_REP_DLANNER2A]
...
------->- PART 1-4 
Declare @TAXONAMT as numeric(12,2),@TAXAMT1 as numeric(12,2),@ITEMAMT as numeric(12,2),@VATMTYPE as varchar(10),@DATE as VARCHAR(6) ,@PARTY_NM as varchar(50),@ADDRESS as varchar(100),@ITEM as varchar(50),@FORM_NM as varchar(30),@S_TAX as varchar(30),@QTY as numeric(18,4),@STTYPE AS  VARCHAR(20),@ITEMTYPE AS VARCHAR(1),@PTTYPE AS VARCHAR(20),@TAX_NAME AS VARCHAR(10)

SELECT @TAXONAMT=0,@TAXAMT1 =0,@ITEMAMT =0,@VATMTYPE ='',@DATE ='',@PARTY_NM ='',@ADDRESS ='',@ITEM ='',@FORM_NM='',@S_TAX ='',@QTY=0,@TAX_NAME=''
...
,date=case when  ( case when @vatfltopt='Bill Date' then month( c.date )  else month(c.date ) END   between 4 and 6 ) then (case when @vatfltopt='Bill Date' then convert(varchar(4),year(c.date))  else convert(varchar(4),year(c.date)) end) +'41' 
 else  case when  ( case when @vatfltopt='Bill Date' then month( c.date )  else month(c.date ) END   between 7 and 9 ) then (case when @vatfltopt='Bill Date' then convert(varchar(4),year(c.date))  else convert(varchar(4),year(c.date)) end) +'42' 
 else case when  ( case when @vatfltopt='Bill Date' then month( c.date )  else month(c.date ) END   between 10 and 12 ) then (case when @vatfltopt='Bill Date' then convert(varchar(4),year(c.date))  else convert(varchar(4),year(c.date)) end) +'43' 
 else case when  ( case when @vatfltopt='Bill Date' then month( c.date )  else month(c.date ) END   between 1 and 3 ) then (case when @vatfltopt='Bill Date' then convert(varchar(4),year(c.date))  else convert(varchar(4),year(c.date)) end) +'44'  end end end end 
,A.PARTY_NM,A.ADDRESS,FORM_NM='',s_tax=a.s_tax ,
--PTTYPE = CASE
...
group by  A.PER,c.u_pinvno,c.u_pinvdt
...
,case when  ( case when @vatfltopt='Bill Date' then month( c.date )  else month(c.date ) END   between 4 and 6 ) then (case when @vatfltopt='Bill Date' then convert(varchar(4),year(c.date))  else convert(varchar(4),year(c.date)) end) +'41' 
 else  case when  ( case when @vatfltopt='Bill Date' then month( c.date )  else month(c.date ) END   between 7 and 9 ) then (case when @vatfltopt='Bill Date' then convert(varchar(4),year(c.date))  else convert(varchar(4),year(c.date)) end) +'42' 
 else case when  ( case when @vatfltopt='Bill Date' then month( c.date )  else month(c.date ) END   between 10 and 12 ) then (case when @vatfltopt='Bill Date' then convert(varchar(4),year(c.date))  else convert(varchar(4),year(c.date)) end) +'43'
 else case when  ( case when @vatfltopt='Bill Date' then month( c.date )  else month(c.date ) END   between 1 and 3 ) then (case when @vatfltopt='Bill Date' then convert(varchar(4),year(c.date))  else convert(varchar(4),year(c.date)) end) +'44' end end end end 
,A.PARTY_NM,A.ADDRESS,c.form_no,A.S_TAX,d.st_type,a.tax_name,a.itemtype, a.u_imporm,c.vatmtype
...
--Add bill date and transaction date wise filter option by sandeep for TKT-9444 -->Start 
-- change by Sandeep for TKT-8983 -->start
select Per,TAXONAMT=sum(TAXONAMT),TAXAMT=SUM(TAXAMT),ITEMAMT=sum(ITEMAMT),VATMTYPE,date,PARTY_NM,ADDRESS,FORM_NM='',s_tax,PTTYPE 
from #1 group by per,PARTY_NM,ADDRESS,VATMTYPE,date,s_tax,PTTYPE 
OPEN CUR_DLVAT30
FETCH NEXT FROM CUR_DLVAT30 INTO @PER,@TAXONAMT,@TAXAMT,@ITEMAMT,@VATMTYPE,@DATE,@PARTY_NM,@ADDRESS,@FORM_NM,@S_TAX,@PTTYPE--,@TAX_NAME
...
BEGIN
	...
	INSERT INTO #DLVAT30 (PART,PARTSR,SRNO,RATE,AMT1,AMT2,AMT3,VATMTYPE,DATE,PARTY_NM,ADDRESS,FORM_NM,S_TAX,PTTYPE)
                 VALUES (1,'1','A',@PER,@TAXONAMT,@TAXAMT,@ITEMAMT,@VATMTYPE,@DATE,@PARTY_NM,@ADDRESS,@FORM_NM,@S_TAX,@PTTYPE)
	
	SET @CHAR=@CHAR+1
	FETCH NEXT FROM CUR_DLVAT30 INTO @PER,@TAXONAMT,@TAXAMT,@ITEMAMT,@VATMTYPE,@DATE,@PARTY_NM,@ADDRESS,@FORM_NM,@S_TAX,@PTTYPE
END
CLOSE CUR_DLVAT30
...
Update #DLVAT30 set  PART = isnull(Part,0) , Partsr = isnull(PARTSR,''), SRNO = isnull(SRNO,''),
 ...
 SELECT * FROM #DLVAT30 order by cast(substring(partsr,1,case when (isnumeric(substring(partsr,1,2))=1) then 2 else 1 end) as int), partsr,SRNO,DATE
End
