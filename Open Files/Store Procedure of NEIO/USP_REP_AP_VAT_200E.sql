If Exists(Select [name] From SysObjects Where xtype='P' and [Name]='USP_REP_AP_VAT_200E')
Begin
	Drop Procedure USP_REP_AP_VAT_200E
End
GO
-- =============================================
-- Author:		Hetal L. Patel
-- Create date: 01/06/2009	
-- Description:	This Stored procedure is useful to generate AP VAT FORM 200E.
-- Modify date: 24/06/2009
-- Modified By: Dinesh 
-- Modified By: Changes has been done by Gaurav Tanna on 07/10/2014 for BUG-23933
-- Remark:
-- =============================================
CREATE Procedure [dbo].[USP_REP_AP_VAT_200E]
...
SELECT @AMTB1=SUM(GRO_AMT)-Sum(TaxAmt), @Per = Per FROM VATTBL WHERE St_Type = 'Local' and Tax_Name like '%VAT%' and s_tax<>'' and (BHENT In ('ST', 'PR', 'DN')) and Per = 4 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SELECT @AMTB2=SUM(GRO_AMT)-Sum(TaxAmt), @Per = Per FROM VATTBL WHERE St_Type = 'Local' and Tax_Name like '%VAT%' and s_tax<>'' and (BHENT In ('ST', 'PR', 'DN')) and Per = 12.50 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SELECT @AMTC1=SUM(GRO_AMT)-Sum(TaxAmt), @Per = Per FROM VATTBL WHERE St_Type = 'Local' and Tax_Name like '%VAT%' and s_tax<>'' and (BHENT In ('ST', 'PR', 'DN')) and Per = 1 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
-- bug-23933 - end here 
