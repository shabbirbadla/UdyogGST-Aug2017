IF EXISTS(SELECT * FROM SYS.OBJECTS WHERE [NAME]='USP_REP_Emp_HR_Bank_Salary_Account_Request_Letter')
BEGIN
DROP PROCEDURE [USP_REP_Emp_HR_Bank_Salary_Account_Request_Letter]
END
GO
...
Create procedure [dbo].[USP_REP_Emp_HR_Bank_Salary_Account_Request_Letter]
...
e.DOL,e.DOJ,e.padd1,e.padd2,e.padd3,e.pcity,e.pstate,e.ppin,e.designation,e.cobanknm,e.cobankacno,
e.cobanknm,e.cobankacno,e.cobankbran,e.comcode
from employeemast   e 
...
where e.loc_code=l.loc_code   and e.employeename=@empnm and e.employeecode=@empcode
