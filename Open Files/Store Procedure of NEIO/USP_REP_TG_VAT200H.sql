If Exists(Select [name] From SysObjects Where xtype='P' and [Name]='USP_REP_TG_VAT_200H')
Begin
	Drop Procedure USP_REP_TG_VAT_200H
End
GO
-- =============================================
-- Author:		Guarav Tanna
-- Create date: 10/10/2014	for BUG-23935 (Initially created same as AP VAT)
-- Description:	This Stored procedure is useful to generate TG VAT FORM 200H.
-- Remark:
-- Modify by  : Sumit Gavate
-- Remark	  : Modify for Bug no - 27971
-- Modify date: 19/04/2015
-- =============================================
CREATE Procedure [dbo].[USP_REP_TG_VAT_200H]
...
SELECT @AMTB1 = ISNUll(SUM(GRO_AMT)-Sum(TaxAmt),0), @Per = Per FROM VATTBL WHERE St_Type = 'Local' and
	Tax_Name like '%VAT%' and (BHENT In ('ST', 'PR', 'DN')) and Per = 4 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SELECT @AMTB2 = ISNUll(SUM(GRO_AMT)-Sum(TaxAmt),0), @Per = Per FROM VATTBL WHERE St_Type = 'Local' and
	Tax_Name like '%VAT%'  and (BHENT In ('ST', 'PR', 'DN')) and Per = 12.50 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
SELECT @AMTC1 = ISNUll(SUM(GRO_AMT)-Sum(TaxAmt),0), @Per = Per FROM VATTBL WHERE St_Type = 'Local' and
	Tax_Name like '%VAT%'  and (BHENT In ('ST', 'PR', 'DN')) and Per = 1 AND (DATE BETWEEN @SDATE AND @EDATE) group by Per
-- bug-23933 - end here
...
INNER JOIN STMAIN ON (STITEM.inv_no = STMAIN.inv_no AND STITEM.Tran_cd = STMAIN.Tran_cd AND STITEM.date = STMAIN.date) INNER JOIN LCODE ON 
(LCODE.ENTRY_TY=STITEM.ENTRY_TY) WHERE IT_MAST.U_GVTYPE like 'Tax Free Sch - I goods' AND STMAIN.u_imporm <> 'Exempt Transaction' 
...
-->- PART2
INSERT INTO #APVAT (Part,SrNo1,Taxable_Amt,taxamt1,taxamt2,taxamt3,taxamt4,tax_name,level1,st_type) VALUES (2,'A',0,0,0,0,0,'',0,'')

--<- PART2

-->- PART3
INSERT INTO #APVAT (Part,SrNo1,Taxable_Amt,taxamt1,taxamt2,taxamt3,taxamt4,tax_name,level1,st_type) VALUES (3,'A',0,0,0,0,0,'',0,'')
--<- PART3
