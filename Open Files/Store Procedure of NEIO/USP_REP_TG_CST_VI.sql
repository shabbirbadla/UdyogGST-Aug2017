If Exists(Select [name] From SysObjects Where xtype='P' and [Name]='USP_REP_TG_CST_VI')
Begin
	Drop Procedure USP_REP_TG_CST_VI
End
GO
/*
EXECUTE USP_REP_TG_CST_VI'','','','04/01/2015','03/31/2016','','','','',0,0,'','','','','','','','','2015-2016',''
*/
-- =============================================
-- Author:		Guarav Tanna
-- Create date: 10/10/2014	for BUG-23935 (Initially created same as AP VAT)
-- Description:	This Stored procedure is useful to generate TG VAT FORM CST VI.
-- Modify by  : Sumit Gavate
-- Remark	  : Modify for Bug no - 27971
-- Modify date: 19/04/2015
 -- =============================================
CREATE Procedure [dbo].[USP_REP_TG_CST_VI]
...
BEGIN
 ...
 DECLARE @PER NUMERIC(12,2),@TAXAMT NUMERIC(12,2),@CHAR INT,@LEVEL NUMERIC(12,2),@Tax_name VARCHAR(30)
 ...
 */ 
 Select @AMTA1 = ISNULL(Sum(GRO_AMT),0) From vattbl where (date between @sdate and @edate) and bhent = 'ST' And tax_name like ('C.S.T. %')
 --Set @AMTA1 = ISNULL(@AMTA1,0)
 INSERT INTO #FORM221(PART,PARTSR,SRNO,RATE,AMT1,AMT2,AMT3,Party_nm) VALUES(1,'1','A',0,@AMTA1,0,0,'')

--2
--LESS : CST Collections
Select @AMTA1 = ISNULL(Sum(TAXAMT),0) From vattbl where (date between @sdate and @edate) and bhent = 'ST' And tax_name like ('C.S.T. %')	
--Set @AMTA1 = ISNULL(@AMTA1,0)
...
--Balance A - B
 ...
 Select @AMTA2 = ISNULL(Sum(AMT1),0) From #Form221 where PartSr = '2' And SrNo In('B')
 --Set @AMTA1 = ISNULL(@AMTA1,0)
 --Set @AMTA2 = ISNULL(@AMTA2,0)
INSERT INTO #FORM221(PART,PARTSR,SRNO,RATE,AMT1,AMT2,AMT3,Party_nm) VALUES(1,'3','C',0,@AMTA1-@AMTA2,0,0,'')
...
---Tax & Taxable Amount of Sales for the period
 SELECT @AMTA1=0,@AMTB1=0,@AMTC1=0,@AMTD1=0,@AMTE1=0,@AMTF1=0,@AMTG1=0,@AMTH1=0,@AMTI1=0,@AMTJ1=0,@AMTK1=0,@AMTL1=0,@AMTM1=0,@AMTN1=0,@AMTO1=0 
 SELECT @CHAR=64,@Tax_name = ''
 DECLARE  CUR_FORM221 CURSOR FOR 
 select distinct level1,tax_name from stax_mas where ST_TYPE='OUT OF STATE' And (Tax_name like '%CST%' OR tax_name like '%C.S.T%')
 AND level1 > 0 AND level1 IN (1,2,3,4,10,12.50,14.50) ORDER BY level1
 OPEN CUR_FORM221
 FETCH NEXT FROM CUR_FORM221 INTO @PER,@Tax_name
 WHILE (@@FETCH_STATUS=0)
 BEGIN
	-- bug-23933 - (instead of FROM #FORM221_1, changed it to FROM VATTBL)  
	begin
		SELECT @AMTA1 = ISNULL(SUM(GRO_AMT),0),@AMTB1 = ISNULL(SUM(TAXAMT),0) FROM vattbl where bhent = 'ST'
		AND ST_TYPE = 'OUT OF STATE' AND (DATE BETWEEN @SDATE AND @EDATE) AND PER=@PER AND tax_name = @Tax_name
		--AND (DATE BETWEEN @SDATE AND @EDATE) And S_tax <> '' AND PER=@PER 
		--SELECT @AMTB1 = ISNULL(SUM(TAXAMT),0)  FROM vattbl where bhent = 'ST' AND ST_TYPE = 'OUT OF STATE'
		--AND (DATE BETWEEN @SDATE AND @EDATE) And S_tax <> '' AND PER=@PER 
  ...
  */
  ...
  SET @AMTK1=@AMTK1+@NETTAX --TOTAL TAX
  SET @CHAR=@CHAR+1
  INSERT INTO #FORM221(PART,PARTSR,SRNO,RATE,AMT1,AMT2,AMT3,Party_nm) VALUES(1,'4','A',@PER,@NETEFF,@NETTAX,0,'')
...
IF NOT EXISTS(select Rate from #FORM221 where Part = 1 AND PARTSR = '4' AND rate = 1)
BEGIN
	INSERT INTO #FORM221(PART,PARTSR,SRNO,RATE,AMT1,AMT2,AMT3,Party_nm) VALUES(1,'4','A',1,0,0,0,'')
END

IF NOT EXISTS(select Rate from #FORM221 where Part = 1 AND PARTSR = '4' AND rate = 2)
BEGIN
	INSERT INTO #FORM221(PART,PARTSR,SRNO,RATE,AMT1,AMT2,AMT3,Party_nm) VALUES(1,'4','A',2,0,0,0,'')
END

IF NOT EXISTS(select Rate from #FORM221 where Part = 1 AND PARTSR = '4' AND rate = 3)
BEGIN
	INSERT INTO #FORM221(PART,PARTSR,SRNO,RATE,AMT1,AMT2,AMT3,Party_nm) VALUES(1,'4','A',3,0,0,0,'')
END

IF NOT EXISTS(select Rate from #FORM221 where Part = 1 AND PARTSR = '4' AND rate = 4)
BEGIN
	INSERT INTO #FORM221(PART,PARTSR,SRNO,RATE,AMT1,AMT2,AMT3,Party_nm) VALUES(1,'4','A',4,0,0,0,'')
END

IF NOT EXISTS(select Rate from #FORM221 where Part = 1 AND PARTSR = '4' AND rate = 10)
BEGIN
	INSERT INTO #FORM221(PART,PARTSR,SRNO,RATE,AMT1,AMT2,AMT3,Party_nm) VALUES(1,'4','A',10,0,0,0,'')
END

IF NOT EXISTS(select Rate from #FORM221 where Part = 1 AND PARTSR = '4' AND rate = 12.50)
BEGIN
	INSERT INTO #FORM221(PART,PARTSR,SRNO,RATE,AMT1,AMT2,AMT3,Party_nm) VALUES(1,'4','A',12.50,0,0,0,'')
END

IF NOT EXISTS(select Rate from #FORM221 where Part = 1 AND PARTSR = '4' AND rate = 14.50)
BEGIN
	INSERT INTO #FORM221(PART,PARTSR,SRNO,RATE,AMT1,AMT2,AMT3,Party_nm) VALUES(1,'4','A',14.50,0,0,0,'')
END

IF NOT EXISTS(select Rate from #FORM221 where Part = 1 AND PARTSR = '4' AND rate = 0)
BEGIN
...
---Total of Tax & Taxable Amount of Sales for the period
 INSERT INTO #FORM221(PART,PARTSR,SRNO,RATE,AMT1,AMT2,AMT3,Party_nm) VALUES(1,'4','ZZ',0,@AMTJ1,@AMTK1,0,'')
 ...
 */
 ...
 INSERT INTO #FORM221 (PART,PARTSR,SRNO,RATE,AMT1,AMT2,AMT3,Party_nm,Inv_no,Date)
 Select 1, '5', '1', 0,ISNULL(A.Net_Amt,0),0,0,B.bank_nm,A.Inv_no,A.Date From vattbl A Inner Join BpMain B On(A.Bhent = B.Entry_ty And A.Tran_cd = B.Tran_cd)
 Where A.Bhent = 'BP' And A.Date Between @SDate And @EDate And B.Party_nm = 'CST PAYABLE' AND B.U_NATURE <> 'CST'
 -- bug-23933 - end here

 IF NOT EXISTS(select DISTINCT PARTSR from #FORM221 where PARTSR = '5')
 BEGIN
	INSERT INTO #FORM221 (PART,PARTSR,SRNO,RATE,AMT1,AMT2,AMT3,Party_nm,Inv_no,Date)
	VALUES (1, '5', '1', 0,0,0,0,'','','')
 END
 
 
 INSERT INTO #FORM221 (PART,PARTSR,SRNO,RATE,AMT1,AMT2,AMT3,Party_nm,Inv_no,Date)
 Select 1, '5A', 'A', 0,ISNULL(A.Net_Amt,0),0,0,A.AC_NAME,A.Inv_no,A.Date From vattbl A Inner Join BpMain B On(A.Bhent = B.Entry_ty And A.Tran_cd = B.Tran_cd)
 Where A.Bhent = 'BP' And A.Date Between @SDate And @EDate And B.Party_nm = 'CST PAYABLE' AND B.U_NATURE = 'CST'

 IF NOT EXISTS(select DISTINCT PARTSR from #FORM221 where PARTSR = '5A')
 BEGIN
	INSERT INTO #FORM221 (PART,PARTSR,SRNO,RATE,AMT1,AMT2,AMT3,Party_nm,Inv_no,Date)
	VALUES (1, '5A', 'A', 0,0,0,0,'','','')
 END

 INSERT INTO #FORM221 (PART,PARTSR,SRNO,RATE,AMT1,AMT2,AMT3,Party_nm,Inv_no,Date)
 Select 1, '5B', 'Z', 0,ISNULL(SUM(AMT1),0),0,0,'','','' From #FORM221 WHERE PARTSR IN('5','5A')
...
--Sales Of Goods OutSide State (Agent/Principal) Otherwise than by way of inter state sale
 ...
 and U_Imporm = 'Consignment Transfer' and Tax_name not like('C.S.T.%')
 INSERT INTO #FORM221(PART,PARTSR,SRNO,RATE,AMT1,AMT2,AMT3,Party_nm) VALUES (1,'6','A',0,@AMTA1,0,0,'')
...
---Branch Transfer
 ...
 Select @AMTA1 = ISNULL(Sum(GRO_AMT),0) From VATTBL where (date between @sdate and @edate) and bhent = 'ST' And St_Type = 'Out Of State'
 and u_Imporm = 'Branch Transfer' and Tax_name not like('C.S.T.%')
 INSERT INTO #FORM221(PART,PARTSR,SRNO,RATE,AMT1,AMT2,AMT3,Party_nm) VALUES(1,'6','B',0,@AMTA1,0,0,'')
 
 ---Balance C = (A+B)
 Select @AMTA1 = ISNULL(Sum(AMT1),0) From #Form221 where (PartSr = '6' And SrNo = 'A')
 Select @AMTA2 = ISNULL(Sum(AMT1),0) From #Form221 where (PartSr = '6' And SrNo = 'B')
 INSERT INTO #FORM221(PART,PARTSR,SRNO,RATE,AMT1,AMT2,AMT3,Party_nm) VALUES (1,'6','C',0,ISNULL(@AMTA1+@AMTA2,0),0,0,'')
...
--Export Sales
 ...
 St_Type = 'Out Of Country' and u_Imporm NOT IN ('Branch Transfer','Consignment Transfer')
 
 Select @AMTA2 = ISNULL(Sum(GRO_AMT),0) From VATTBL where (date between @sdate and @edate) and bhent = 'EI'
 
 INSERT INTO #FORM221(PART,PARTSR,SRNO,RATE,AMT1,AMT2,AMT3,Party_nm) VALUES(1,'7','A',0,@AMTA1,@AMTA2,@AMTA1 + @AMTA2,'')
 ...
 */
 ...
 Set @AMTA1 = ISNULL(@AMTA1,0)
 Select @AMTA1 = ISNULL(Sum(GRO_AMT),0) From vattbl where (date between @sdate and @edate) and bhent = 'ST' And Tax_name in ('E - 1', 'E - 2') And St_Type = 'Out Of State'
-- Set @AMTA1 = ISNULL(@AMTA1,0)
