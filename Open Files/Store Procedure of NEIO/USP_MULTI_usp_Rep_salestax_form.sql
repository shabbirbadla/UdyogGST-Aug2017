If Exists(Select [name] From SysObjects Where xtype='P' and [Name]='USP_MULTI_usp_Rep_salestax_form')
Begin
	Drop Procedure USP_MULTI_usp_Rep_salestax_form
End
GO
-- =============================================
...
-- ============================================= 
CREATE Procedure [dbo].[USP_MULTI_usp_Rep_salestax_form]
...
BEGIN	
	...
	begin
		...
		set @SQLCOMMAND =''
		SET @SQLCOMMAND='INSERT INTO ##stax_form_rep ('+@IntoStr+@SQL_TMPFLD+' )
						 Execute '+rtrim(@dbname)+'..usp_Rep_salestax_form '+char(39)+@vformnm+char(39)+','+char(39)+@vparty+char(39)+','+char(39)+@mCondn+char(39)+','+cast(@vform as varchar)+','+char(39)+cast(@sdate as varchar)+char(39)+','+char(39)+cast(@edate as varchar)+char(39)+','+char(39)+@dept+char(39)+','+char(39)+@cate+char(39)+','+char(39)+@broker+char(39)
		Execute sp_executesql @sqlcommand		
		update ##stax_form_rep set dbname = rtrim(@dbname),CompNm=rtrim(@co_name),stax_no=rtrim(@stax_no) WHERE ISNULL(dbname,'')=''
		fetch next from ##temp_db into @co_name,@dbname,@stax_no		
	end	
...
SET @WSQL='SELECT dbname,CompNm,stax_no,entry_ty,tran_cd,inv_no,form_nm,form_no,date,net_amt,tax_name,taxamt,mailname,party_nm,formname,rformname
	,bcode_nm,code_nm,add1,add2,add3,contact,city,zip,state,formidt,formrdt,u_pinvno,u_pinvdt '+@xSQL_TMPFLD+' INTO '+@tmpTbl+'  FROM  ##stax_form_rep '	

EXECUTE SP_EXECUTESQL @WSQL

	DROP TABLE ##stax_form_rep
